...
class DescrptSeAtten(DescrptSeA):
    ...
    def compute_input_stats(self,
        ...
        natoms_vec
                ...
                If large_batch_mode is True, this para is blank. See real_natoms_vec.
        mesh
                The mesh for neighbor searching. Can be generated by deepmd.model.make_stat_input
        input_dict
                Dictionary for additional input
        large_batch_mode
                Perform the large batch mode or not.
                If True, it means that a new system contains data from different systems originally with same nloc.
        real_natoms_vec
                If large_batch_mode is True, it takes in the real natoms_vec.
        """
        all_davg = []
        all_dstd = []
        if True:
            ...
            if large_batch_mode:
                ...
                for cc, bb, tt, nn, mm, r_n in zip(data_coord, data_box, data_atype, natoms_vec, mesh, real_natoms_vec):
                    sysr, sysr2, sysa, sysa2, sysn \
                        = self._compute_dstats_sys_smth(cc, bb, tt, nn, mm, large_batch_mode, r_n)
                    sys_num += 1
                    sumr.append(sysr)
                    suma.append(sysa)
                    sumn.append(sysn)
    ...
    def build(self,
        ...
        self.negtive_mask = -(2 << 32) * (1.0 - self.nmask)
