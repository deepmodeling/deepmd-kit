...
from deepmd.dpa_tools.data.loader import _get_source, _resolve_label_key

_LOG = logging.getLogger("dpa_tools.cv")
...
def _build_sklearn_head(predictor_type: str, seed: int = 42):
    ...
    Delegates to ``dpa_tools.utils.sklearn_heads.build_sklearn_head``.
    """
...
def _assemble_from_per_system_cache(
    ...
    y : np.ndarray (1D)
    """
    from deepmd.dpa_tools.data.desc_cache import get_per_system_descriptor
    X_list, y_list = [], []
...
def cross_validate(
    ...
    if is_cheap:
        from deepmd.dpa_tools.data.desc_cache import ensure_per_system_cache
