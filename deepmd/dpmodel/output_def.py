...
class PropertyMetadataHolder:
    """Mixin for exported artifacts that carry persisted property metadata.

    The JAX ``HLO`` and TF2 SavedModel wrappers hold a compiled artifact with
    no live model, so they cannot compute the property name/dimension/
    intensiveness from a fitting net (as the dpmodel ``PropertyModel`` does).
    They instead read the values that were persisted at export time. This mixin
    provides the accessors and the property ``OutputVariableDef`` builder; the
    subclass only has to set ``_var_name``/``_task_dim``/``_intensive`` (a
    non-property model leaves them at their defaults).
    """

    _var_name: str | None = None
    _task_dim: int | None = None
    _intensive: bool = False
    ...
    def _property_output_var_def(self) -> OutputVariableDef:
        """Rebuild the property ``OutputVariableDef`` from the persisted metadata."""
