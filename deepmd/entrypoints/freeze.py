...
def _remove_fitting_net_suffix(sess, output_graph_def, out_suffix):
...
def freeze(
    *, checkpoint_folder: str, output: str, node_names: Optional[str] = None, nvnmd_weight: Optional[str] = None, **kwargs
    ...
    def freeze_graph(_sess, input_graph, input_node, freeze_type, modifier, out_graph_name, _node_names=None, out_suffix=''):
    ...
    with tf.Session() as sess:
        ...
        if model_type != 'multi-task':
            ...
            for fitting_key in input_script['model']['fitting_net_dict']:
                ...
                if output_graph[-3:] == '.pb':
                    output_graph_item = output_graph[:-3] + '_{}.pb'.format(fitting_key)
                else:
                    output_graph_item = output_graph + '_{}'.format(fitting_key)
                freeze_graph(sess, input_graph_def, nodes, fitting_type, modifier_type, output_graph_item, node_names,
