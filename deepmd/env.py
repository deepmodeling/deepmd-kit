...
def get_tf_default_nthreads():
    return int(os.environ.get('TF_INTRA_OP_PARALLELISM_THREADS')), int(
        os.environ.get('TF_INTER_OP_PARALLELISM_THREADS'))
...
def get_module(module_name):
    ...
    assert (os.path.isfile(os.path.join(module_path, "{}.{}".format(module_name, ext)))), "module %s does not exist" % module_name
    module = tf.load_op_library(os.path.join(module_path, "{}.{}".format(module_name, ext)))
    return module
