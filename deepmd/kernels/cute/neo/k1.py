...
class NeoK1RuntimeConfig:
    """Architecture-specific choices for the compact Neo K1 path."""
...
@dataclass(frozen=True)
class NeoK1Spec:
    """Shape and branch contract for one Neo K1 replacement."""
...
def is_cute_k1_enabled() -> bool:
    """Return whether the Neo CuTe K1 path is enabled by environment."""
    return runtime_policy.is_cute_infer_enabled()
...
def get_neo_k1_spec(block: Any) -> NeoK1Spec:
    """Extract the K1 contract from a SeZM interaction block."""
    so2 = block.so2_conv
    return NeoK1Spec(
        lmax=int(so2.lmax),
...
def is_supported_neo_k1_block(block: Any) -> bool:
    """Return whether this block can use the current Neo CuTe K1 path."""
    so2 = block.so2_conv
    return (
        get_neo_k1_spec(block).is_current_neo_target
        and bool(so2.focus_norm)
        and not bool(so2.edge_cartesian)
        and getattr(so2, "node_cartesian_tp", None) is None
        and type(block.post_so2_norm) is EquivariantRMSNorm
...
def packed_wigner_edges_eligible(
    *,
    ...
    return (
        ...
        and runtime_policy.k1_int32_indexing_is_safe(
...
def is_neo_k1_static_eligible(
    ...
    *,
    ...
    """Check K1 conditions that are stable during one descriptor forward."""
    return (
        is_cute_k1_enabled()
        and not training
        and not bool(getattr(block, "training", False))
        and device.type == "cuda"
        and _device_is_supported_for_k1(device)
        ...
        and getattr(block, "_deepmd_cute_k1_state", None) is not False
        and is_supported_neo_k1_block(block)
    )


def is_neo_k1_runtime_eligible(
    block: Any,
    *,
    training: bool,
    device: torch.device,
    ...
    """Return the exact strict-FP32 inference contract for K1 dispatch."""
    return (
        edge_count > 0
        and destinations_sorted
        and is_neo_k1_static_eligible(
            block,
            training=training,
            device=device,
            dtype=dtype,
...
def is_packed_wigner_candidate(
    *,
    ...
    if has_edge_src_gate:
        # K1 currently falls back for SFPG so eager must receive dense Wigner data.
        return False
    ...
    if device.type == "cuda":
        try:
            ...
            packed_wigner_enabled = (
                is_cute_k1_enabled()
                and runtime_policy.is_supported_k1_capability(compute_capability)
            )
    else:
        packed_wigner_enabled = False
    if (
...
class _RegistryEntry:
    ...
    def block(self) -> Any:
        ...
        if block is None:
            raise RuntimeError("the registered Neo K1 block has been released")
        return block


@dataclass(frozen=True)
class _RegisteredK1State:
    device_index: int
    handle: int
    config: NeoK1RuntimeConfig
...
def _compile_output_gate_backward(
    ...
    from .k1_kernels.cute_neo_output_gate_backward import (
...
def _validate_gate_expand_index(block: Any) -> None:
    ...
    if not _gate_expand_index_is_supported(block):
        raise ValueError(
            "Neo K1 fused gate kernels require expand_index=[0,1,2,0,1,2,0,1,2]"
        )


@torch.compiler.disable
def _register_cute_k1_state(
    block: Any,
    device_index: int,
    config: NeoK1RuntimeConfig,
) -> _RegisteredK1State | None:
    """Publish cold K1 state eagerly before its handle enters a custom op."""
    ...
    if not _module_state_is_aligned(block):
        ...
        block._deepmd_cute_k1_state = False
        return None
    state = _RegisteredK1State(
        device_index=device_index,
        handle=register_cute_k1_block(block, config),
        config=config,
    )
    block._deepmd_cute_k1_state = state
    return state


@torch.compiler.disable
def prepare_cute_k1_blocks(
    blocks: Any,
    *,
    training: bool,
    device: torch.device,
    ...
    """Validate and register K1 state before model graph capture begins."""
    ...
    if not is_supported_k1_compute_capability(compute_capability):
        return False
    ...
    if not all(
        ...
        return False
    return all(
        _register_cute_k1_state(block, device_index, config) is not None
        for block in block_tuple
    )
...
def _runner_token_key(
    ...
    if runner_token.dtype != torch.uint8 or runner_token.numel() != 1:
        raise ValueError(f"{path} Neo K1 runner token must be one uint8 value")
    return (
...
def _store_packed_runner(runner_token: Tensor, runner: Any) -> None:
    ...
    with _PACKED_RUNNER_CACHE_LOCK:
        ...
        if old_state is not None:
            raise RuntimeError("packed Neo K1 runner token is already outstanding")
        else:
...
def _borrow_packed_runner(runner_token: Tensor) -> Any | None:
    ...
    if state is None:
        raise RuntimeError("packed Neo K1 runner token is not outstanding")
...
def _assert_grad_meta_contract(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError(
            f"Neo K1 {name} gradient violates the custom-op meta contract: "
...
def _assert_not_cuda_graph_capturing(path: str, tensor: Tensor) -> None:
    ...
    if tensor.is_cuda and torch.cuda.is_current_stream_capturing():
        raise RuntimeError(
            f"Neo K1 {path} cannot run inside direct CUDA graph capture; "
...
def _fake_x_wide_grad_like(x: Tensor, *, skip: bool) -> Tensor:
    """Fake the native K1 grad-x layout when layout restore is skipped.
...
def _equivariant_rmsnorm_backward(norm: Any, x: Tensor, grad_out: Tensor) -> Tensor:
    ...
    in_dtype = x.dtype
    xf = x.to(dtype=norm.dtype)
    gf = grad_out.to(dtype=norm.dtype)
    x0_in = xf[:, :1, :, :]
    xt = xf[:, 1:, :, :]
    ...
    inv = torch.rsqrt(mean_variance + norm.eps_tensor).unsqueeze(1).unsqueeze(-1)
...
def _neo_so2_linear_backward_input_with_residual(
    ...
    *,
    ...
    raise RuntimeError("Neo K1 SO2 backward reached an unsupported storage pattern")
...
def _build_runner(
    ...
    if config.native_sm90_path:
        ...
        from .k1_runner import NeoFullCuteBackward as Runner
...
def _k1_packed_direct_forward_impl(
    handle: int,
    x: Tensor,
    d_full: Tensor,
    dt_full: Tensor,
...
def _k1_backward_from_runner_current_device(
    handle: int,
    grad_out: Tensor,
    x: Tensor,
    d_full: Tensor,
...
def _k1_backward_from_runner(
    ...
    with torch.cuda.device(grad_out.device):
        return _k1_backward_from_runner_current_device(
            handle,
            grad_out,
            x,
            d_full,
...
def _k1_packed_direct_backward_impl(
    handle: int,
    grad_out: Tensor,
    x: Tensor,
    d_full: Tensor,
    ...
    return _k1_backward_from_runner(
        handle,
        grad_out,
        x,
        d_full,
...
def _runner_backward_manual(
    ...
    # Keep the large K1 slabs out of the readout-backward allocation crest.
    runner.ensure_backward_workspace()

    from .k1_kernels.cute_neo_phase_c_backward_layout_runner import (
    ...
    from .k1_radial_phase_a_node import (
...
_K1_CUSTOM_OP_TAGS = _stateful_custom_op_tags()
_k1_packed_direct_op = torch.library.custom_op(
    "sezm_cute::k1_packed_direct", mutates_args=(), tags=_K1_CUSTOM_OP_TAGS
)(_k1_packed_direct_forward_impl)
_k1_packed_direct_bwd_op = torch.library.custom_op(
    "sezm_cute::k1_packed_direct_bwd", mutates_args=(), tags=_K1_CUSTOM_OP_TAGS
)(_k1_packed_direct_backward_impl)


@_k1_packed_direct_op.register_fake
def _(
    handle: int,
    x: Tensor,
    d_full: Tensor,
...
@_k1_packed_direct_bwd_op.register_fake
def _(
    handle: int,
    grad_out: Tensor,
    x: Tensor,
...
def _k1_packed_direct_setup_context(
    ctx: Any,
    inputs: tuple[Any, ...],
...
def _k1_packed_direct_registered_backward_impl(
    ctx: Any,
    grad_out: Tensor,
) -> tuple[Any, ...]:
    (
    ...
    grad_x, grad_d, grad_dt, grad_radial, grad_edge_env = _k1_packed_direct_bwd_op(
        ctx.handle,
        grad_out,
        x,
        d_full,
...
def _k1_packed_direct_backward(
    ctx: Any,
    grad_out: Tensor,
    ...
    return _k1_packed_direct_registered_backward_impl(ctx, grad_out)


_k1_packed_direct_op.register_autograd(
    _k1_packed_direct_backward,
    setup_context=_k1_packed_direct_setup_context,
)
...
def _cute_k1_impl(
    ...
    output, _runner_token = _k1_packed_direct_op(
        int(handle),
        x,
        d_full,
        dt_full,
...
def _maybe_run_prepared_cute_k1(
    block: Any,
    x: Tensor,
    edge_cache: Any,
    radial_feat: Tensor,
    ...
    """Dispatch prevalidated packed K1 state without a Python graph break."""
    state = getattr(block, "_deepmd_cute_k1_state", None)
    ...
    if not runtime_policy.k1_int32_indexing_is_safe(
        edge_count=edge_count,
...
@torch.compiler.disable
def _maybe_run_cute_k1_fallback(
    block: Any,
    x: Tensor,
    edge_cache: Any,
    radial_feat: Tensor,
    ...
    """Run the opt-in Neo CuTe K1 path, or return ``None`` for fallback."""
    ...
    if edge_cache.edge_src_gate is not None:
        return None
    destinations_sorted = bool(getattr(edge_cache, "destinations_sorted", False))
    if not is_neo_k1_runtime_eligible(
        block,
    ...
    if state is False:
        return None
    if state is not None and not isinstance(state, _RegisteredK1State):
        state = None
    device_index = x.device.index
    if device_index is None:
        device_index = torch.cuda.current_device()
    ...
    if state is None or state.device_index != device_index or state.config != config:
        state = _register_cute_k1_state(block, device_index, config)
        if state is None:
            return None
...
def maybe_run_cute_k1(
    ...
    use_prepared = isinstance(
        state,
        _RegisteredK1State,
    ) or runtime_policy.is_k1_thin_wrapper_enabled(_tensor_compute_capability(x))
    if use_prepared:
        output = _maybe_run_prepared_cute_k1(
            block,
            x,
            edge_cache,
            radial_feat,
        ...
        if output is not None:
            return output
    return _maybe_run_cute_k1_fallback(
        block,
        x,
        edge_cache,
        radial_feat,
