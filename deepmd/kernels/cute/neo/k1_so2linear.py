...
"""SO2Linear helpers for the Neo CuTe K1 path.
...
def _validate_neo_so2_linear(so2_linear: Any) -> None:
    if (
        ...
        raise NotImplementedError(
            "Neo CuTe K1 SO2Linear expects lmax=3,mmax=1,F=2,C=32"
        )
...
def _validate_edge_focus(tensor: Tensor, name: str) -> None:
    ...
    if not tensor.is_contiguous():
        raise ValueError(f"{name} must use the canonical contiguous K1 layout")
