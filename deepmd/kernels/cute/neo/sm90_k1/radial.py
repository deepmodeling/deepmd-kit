...
"""Compact radial state for the SM90 persistent-complex Neo K1 path.
...
def run_neo_radial_state_forward_fp32(
    *,
    ...
    if edge_count <= 0:
        raise ValueError("persistent-complex K1 requires E > 0")
    if tuple(torch.cuda.get_device_capability(device)) != (9, 0):
        raise RuntimeError("persistent-complex K1 requires SM90")
