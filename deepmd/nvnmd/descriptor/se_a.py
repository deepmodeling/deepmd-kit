  ...
  with tf.variable_scope('filter_type_all_x', reuse=True):
    ...
    for type_i in range(ntypes):
      ...
      for ii in range(len(keys)):     
        map_outs.append(op_module.map_nvnmd(u_i, \
...
def filter_lower_R42GR(
  ...
  """:se_a.py/DescrptSeA/_filter_lower
  """
  ...
  if (nvnmd_cfg.quantize_descriptor):
    ...
    for ii in range(len(keys)):
      with tf.variable_scope(keys[ii], reuse=True):
        map_outs.append(op_module.map_nvnmd(s-s_min, \
          map_tables[ii][0], map_tables[ii][1] / prec, \
          map_tables2[ii][0], map_tables2[ii][1] / prec, \
          prec, NBIT_FEA_FL))
    ...
    if (not is_exclude):
        ...
        if (not uniform_seed) and (seed is not None): seed += seed_shift
    else:
...
def filter_GR2D(xyz_scatter_1):
  ...
  if (nvnmd_cfg.quantize_descriptor):
    ...
    xyz_scatter_1 = op_module.quantize_nvnmd(xyz_scatter_1, 0, -1, NBIT_DATA_FL, -1)  # fix the number of bits of gradient 
