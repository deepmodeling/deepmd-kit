...
class Wrap():
    ...
    def wrap(self):
        ...
        hcfg = e.extend_list(hcfg, int(np.ceil(len(hcfg) / NSTDM_M1X)) * NSTDM_M1X) # 长度为NSTDM_M1X的倍数，使得data_merge部分可以正常运行
        ...
        for ss in range(NSEL):
            ...
            for ll in range(nlayer_fit):
                ...
                if (nc == 1):
                    #final layer
                    #* fp *#
                    bi = [bw[ll][tt][sr*nrs+rr][cc] for rr in range(nrs) for cc in range(nc)]; bi.reverse(); bfp = ''.join(bi) + bfp
                    bi = [bb[ll][tt][sc*ncs*0+cc] for cc in range(ncs)]; bi.reverse(); bfp = ''.join(bi) + bfp
                    #* bp *#
                    bi = [bw[ll][tt][sr*nrs+rr][cc] for rr in range(nrs) for cc in range(nc)]; bi.reverse(); bbp = ''.join(bi) + bbp
                    bi = [bb[ll][tt][sc*ncs*0+cc] for cc in range(ncs)]; bi.reverse(); bbp = ''.join(bi) + bbp
                else:
                    #* fp *#
                    bi = [bw[ll][tt][rr][sc*ncs+cc] for cc in range(ncs) for rr in range(nr)]; bi.reverse(); bfp = ''.join(bi) + bfp
    ...
    def wrap_map(self):
        ...
        dat = np.hstack(dat)
        dat = dat[:,::-1]
        dat = dat[:, idx] # value 和 delta_value 放在一起
