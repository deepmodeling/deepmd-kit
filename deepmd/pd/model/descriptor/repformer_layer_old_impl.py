...
def _apply_h_norm(
    hh: paddle.Tensor,  # nf x nloc x nnei x 3
) -> paddle.Tensor:
...
class RepformerLayer(paddle.nn.Layer):
    ...
    def _update_g1_conv(
        ...
        if not self.smooth:
            ...
            invnnei = (1.0 / float(nnei)) * paddle.ones(
                (nb, nloc, 1), dtype=env.GLOBAL_PD_FLOAT_PRECISION
            ).to(device=gg1.place)
        # nb x nloc x ng2
    ...
    def _apply_nb_1(self, bn_number: int, gg: paddle.Tensor) -> paddle.Tensor:
        nb, nl, nf = gg.shape
        gg = gg.reshape([nb, 1, nl * nf])
        if bn_number == 1:
    ...
    def _apply_nb_2(
        ...
        if bn_number == 1:
            ...
            assert self.bn2 is not None
            gg = self.bn2(gg)
        return gg.reshape([nb, nl, nnei, nf])
    ...
    def _apply_bn_comp(
        ...
        if bn_number == 1:
            assert self.bn1 is not None
            gg = self.bn1(gg).reshape([ss])
        else:
            assert self.bn2 is not None
            gg = self.bn2(gg).reshape([ss])
        return gg
