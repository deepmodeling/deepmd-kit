...
class DropPath(paddle.nn.Layer):
    ...
    def forward(self, x):
        ...
        random_tensor: paddle.Tensor = keep_prob + paddle.rand(shape, dtype=x.dtype).to(
            device=x.place
        )
...
def softmax_dropout(
    ...
    if bias is not None:
        input_x += bias
    return F.dropout(F.softmax(input_x, axis=-1), p=dropout_prob, training=is_training)
...
def checkpoint_sequential(
    ...
    if enabled and is_grad_enabled:
        for func in functions:
            input_x = paddle.distributed.fleet.utils.recompute(
                get_wrap_exec(func), *input_x
            )
    else:
        for func in functions:
...
class ResidualLinear(nn.Layer):
    ...
    def forward(self, inputs):
        """Return X ?+ X*W+b."""
        xw_plus_b = paddle.matmul(inputs, self.matrix) + self.bias
        hidden = paddle.tanh(xw_plus_b)
        if self.resnet:
            hidden = hidden * self.idt
        if self.num_in == self.num_out:
            return inputs + hidden
        elif self.num_in * 2 == self.num_out:
            return paddle.concat([inputs, inputs], axis=1) + hidden
        else:
            return hidden


class TypeFilter(nn.Layer):
    use_tebd: Final[bool]
    tebd_mode: Final[str]
    ...
    def __init__(
        ...
        self.deep_layers_t = nn.LayerList(deep_layers_t)
    ...
    def forward(
        self,
        inputs,
        atype_tebd: Optional[paddle.Tensor] = None,
        nlist_tebd: Optional[paddle.Tensor] = None,
    ):
        ...
        Returns
        -------
        - `paddle.Tensor`: Embedding contributed by me. Its shape is [nframes*natoms[0], 4, self.neuron[-1]].
        """
        ...
        if self.use_tebd and self.tebd_mode == "concat":
            ...
            xyz_scatter = paddle.concat([xyz_scatter, nlist_tebd, atype_tebd], axis=1)
        ...
        if self.use_tebd and self.tebd_mode in [
            ...
            two_side_tebd = paddle.concat([nlist_tebd, atype_tebd], axis=1)
        ...
        xyz_scatter = xyz_scatter.reshape(
        ...
        if self.return_G:
            ...
            inputs_reshape = inputs_i.reshape([-1, self.length, 4]).transpose([0, 2, 1])
            return paddle.matmul(inputs_reshape, xyz_scatter)
...
class GaussianKernel(nn.Layer):
    def __init__(self, K=128, num_pair=512, std_width=1.0, start=0.0, stop=9.0):
        ...
        start = start
        stop = stop
        mean = paddle.linspace(start, stop, K, dtype=env.GLOBAL_PD_FLOAT_PRECISION)  # pylint: disable=no-explicit-device
...
class GaussianEmbedding(nn.Layer):
    def __init__(
        self,
        rcut,
        kernel_num,
        ...
        if self.atomic_sum_gbf:
            if kernel_num != self.embed_dim:
                self.edge_proj = paddle.nn.Linear(
                    kernel_num, self.embed_dim, dtype=env.GLOBAL_PD_FLOAT_PRECISION
                )
            else:
                self.edge_proj = None
        self.ntypes = ntypes
    ...
    def forward(self, coord_selected, atom_feature, edge_type_2dim, edge_feature):
        ...
        if self.atomic_sum_gbf:
            ...
            # [ncluster, natoms, K]
            sum_edge_features = edge_features.sum(axis=-2)
...
class NeighborWiseAttention(nn.Layer):
    def __init__(
        self,
        layer_num,
        nnei,
...
class NeighborWiseAttentionLayer(nn.Layer):
    ffn: Final[bool]
...
class GatedSelfAttetion(nn.Layer):
    def __init__(
        self,
        nnei,
        embed_dim,
...
class LocalSelfMultiheadAttention(nn.Layer):
    ...
    def forward(
        ...
        nlist_mask: Optional[paddle.Tensor] = None,
        ...
        # padding = paddle.zeros(feature_dim, dtype=env.GLOBAL_PD_FLOAT_PRECISION).to(k.place)
        # k = paddle.concat([k, padding.unsqueeze(0).unsqueeze(1)], axis=1)
        ...
        v = (
            v.reshape([nframes, nloc, nnei, self.attn_head, self.head_dim])
            .transpose([0, 3, 1, 2, 4])
            .contiguous()
            .reshape([nframes * self.attn_head * nloc, nnei, self.head_dim])
        )
        ...
        # softmax
        # [nframes * attn_head * nloc, 1, nnei]
        attn = F.softmax(attn_weights, axis=-1).reshape(
...
class NodeTaskHead(nn.Layer):
    ...
    def forward(
        ...
        ).reshape([ncluster, self.num_heads, natoms, natoms])
        ...
        x = x.transpose([0, 3, 2, 1, 4]).contiguous().reshape([ncluster, natoms, 3, -1])
        cur_force = self.force_proj(x).reshape([ncluster, natoms, 3])
        return cur_force


class EnergyHead(nn.Layer):
    def __init__(
        self,
        input_dim,
        output_dim,
...
class OuterProduct(nn.Layer):
    ...
    def _opm(self, a, b):
        ...
        a = a.reshape([nframes, nloc, 1, d, 1])
        b = b.reshape([nframes, 1, nloc, 1, d])
...
class TriangleMultiplication(nn.Layer):
    ...
    def forward(
        ...
        g = self.linear_g(z)
        if self.training:
            ab = self.linear_ab_p(z) * paddle.sigmoid(self.linear_ab_g(z))
        else:
            ab = self.linear_ab_p(z)
            ab *= paddle.sigmoid(self.linear_ab_g(z))
        # [nframes, nloc, nloc, d]
        a, b = paddle.chunk(ab, 2, axis=-1)
        del z, ab
...
class EvoformerEncoderLayer(nn.Layer):
    def __init__(
        self,
        feature_dim: int = 768,
        ffn_dim: int = 2048,
...
class Evoformer2bEncoder(nn.Layer):
    def __init__(
        self,
        nnei: int,
        layer_num: int = 6,
    ...
    def forward(self, atomic_rep, pair_rep, nlist, nlist_type, nlist_mask):
        ...
        # [nframes, attn_head, nloc, nnei]
        pair_rep = pair_rep.transpose([0, 3, 1, 2]).contiguous()
        ...
        def norm_loss(x, eps=1e-10, tolerance=1.0):
            ...
            norm = paddle.sqrt(paddle.sum(x**2, axis=-1) + eps)
            error = F.relu((norm - max_norm).abs() - tolerance)
            return error

        def masked_mean(mask, value, dim=-1, eps=1e-10):
            return (
                paddle.sum(mask * value, axis=dim) / (eps + paddle.sum(mask, axis=dim))
            ).mean()

        # atomic_rep shape: [nframes, nloc, feature_dim]
        # pair_rep shape: [nframes, attn_head, nloc, nnei]

        norm_x = paddle.mean(norm_loss(atomic_rep))
        if self._final_layer_norm:
        ...
        # [nframes, nloc, nnei, attn_head]
        delta_pair_rep = (
            delta_pair_rep.reshape([nframes, self.attn_head, nloc, nnei])
            .transpose([0, 2, 3, 1])
            .contiguous()
        )
...
class Evoformer3bEncoderLayer(nn.Layer):
    def __init__(
        self,
        nnei,
    ...
    def forward(
        ...
        x: paddle.Tensor,
        pair: paddle.Tensor,
        nlist: paddle.Tensor = None,
        attn_mask: Optional[paddle.Tensor] = None,
        pair_mask: Optional[paddle.Tensor] = None,
        op_mask: float = 1.0,
        op_norm: float = 1.0,
    ):
        """Encoder the atomic and pair representations.
...
class Evoformer3bEncoder(nn.Layer):
    def __init__(
        self,
        nnei,
        layer_num=6,
        ...
        **kwargs,
        ...
        if droppath_prob > 0:
            droppath_probs = [
                x.item()
                for x in paddle.linspace(0, droppath_prob, layer_num)  # pylint: disable=no-explicit-dtype,no-explicit-device
            ]
        else:
            droppath_probs = None
