...
class FittingNetAttenLcc(Fitting):
    ...
    def forward(self, output, pair, delta_pos, atype, nframes, nloc):
        ...
        # energy outut
        # [nframes, nloc]
        energy_out = self.engergy_proj(output_nloc).reshape([nframes, nloc])
        # [nframes, nloc]
        energy_factor = self.energe_agg_factor(paddle.zeros_like(atype)).reshape(
            [nframes, nloc]
        )
        energy_out = (energy_out * energy_factor) + self.bias_atom_e[atype]
        energy_out = energy_out.sum(axis=-1)
