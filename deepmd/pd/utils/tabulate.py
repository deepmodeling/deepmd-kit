...
class DPTabulate(BaseTabulate):
    ...
    def __init__(
        ...
        if isinstance(self.descrpt, deepmd.pd.model.descriptor.DescrptDPA2):
    ...
    def _make_data(self, xx, idx):
        ...
        for layer in range(self.layer_size):
            if layer == 0:
                xbar = paddle.matmul(
                    xx,
                    paddle.to_tensor(self.matrix["layer_" + str(layer + 1)][idx]).to(
                        env.DEVICE
                    ),
                ) + paddle.to_tensor(self.bias["layer_" + str(layer + 1)][idx]).to(
                    env.DEVICE
                )
                if self.neuron[0] == 1:
                    yy = (
                    ...
                    dy = unaggregated_dy_dx_s(
                        ...
                        xbar,
                        self.functype,
                    ) + paddle.ones((1, 1), dtype=yy.dtype)  # pylint: disable=no-explicit-device
                    ...
                    dy = unaggregated_dy_dx_s(
                        ...
                        xbar,
                        self.functype,
                    ) + paddle.ones((1, 2), dtype=yy.dtype)  # pylint: disable=no-explicit-device
                    dy2 = unaggregated_dy2_dx_s(
                        yy - tt,
                        dy,
                        self.matrix["layer_" + str(layer + 1)][idx],
    ...
    def _layer_0(self, x, w, b):
        ...
        return self.activation_fn(paddle.matmul(x, w) + b)
    ...
    def _get_descrpt_type(self) -> str:
        if isinstance(
            self.descrpt,
            (
                deepmd.pd.model.descriptor.DescrptDPA1,
                deepmd.pd.model.descriptor.DescrptDPA2,
            ),
        ):
            return "Atten"
        elif isinstance(self.descrpt, deepmd.pd.model.descriptor.DescrptSeA):
            return "A"
        elif isinstance(self.descrpt, deepmd.pd.model.descriptor.DescrptSeR):
            return "R"
        elif isinstance(self.descrpt, deepmd.pd.model.descriptor.DescrptSeT):
            return "T"
...
# customized op
def grad(xbar: paddle.Tensor, y: paddle.Tensor, functype: int):
    if functype == 1:
        return 1 - y * y

    elif functype == 2:
        var = paddle.tanh(SQRT_2_PI * (xbar + GGELU * xbar**3))
        return (
        ...
        return paddle.where(xbar > 0, paddle.ones_like(xbar), paddle.zeros_like(xbar))

    elif functype == 4:
        return paddle.where(
            (xbar > 0) & (xbar < 6), paddle.ones_like(xbar), paddle.zeros_like(xbar)
        )

    elif functype == 5:
        return 1.0 - 1.0 / (1.0 + paddle.exp(xbar))
...
def grad_grad(xbar: paddle.Tensor, y: paddle.Tensor, functype: int):
    if functype == 1:
        return -2 * y * (1 - y * y)

    elif functype == 2:
        var1 = paddle.tanh(SQRT_2_PI * (xbar + GGELU * xbar**3))
        ...
        return paddle.zeros_like(xbar)

    elif functype == 5:
        exp_xbar = paddle.exp(xbar)
        ...
        return -paddle.ones_like(xbar)


def unaggregated_dy_dx_s(
    y: paddle.Tensor, w_np: np.ndarray, xbar: paddle.Tensor, functype: int
):
    w = paddle.to_tensor(w_np).to(env.DEVICE)
    y = y.to(env.DEVICE)
