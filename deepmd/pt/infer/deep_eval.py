    ...
    def _eval_model(
        ...
        for odef in request_defs:
            ...
            if pt_name in batch_output:
                ...
                shape = self._get_output_shape(odef, nframes, natoms)
                results.append(np.full(np.abs(shape), np.nan))  # pylint: disable=no-explicit-dtype; this is kinda hack
    ...
    def _eval_model_spin(
        ...
        for odef in request_defs:
            ...
            if pt_name in batch_output:
                ...
                shape = self._get_output_shape(odef, nframes, natoms)
                results.append(np.full(np.abs(shape), np.nan))  # pylint: disable=no-explicit-dtype; this is kinda hacky
        return tuple(results)
...
def eval_model(
    ...
    if not return_tensor:
        ...
        atomic_energy_out = (
            np.concatenate(atomic_energy_out)  # pylint: disable=no-explicit-dtype
        ...
        force_mag_out = (
            np.concatenate(force_mag_out)  # pylint: disable=no-explicit-dtype
            if force_mag_out
        ...
        atomic_virial_out = (
            np.concatenate(atomic_virial_out)  # pylint: disable=no-explicit-dtype
