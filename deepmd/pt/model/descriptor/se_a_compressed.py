...
class DescrptSeA(BaseDescriptor, torch.nn.Module):
    ...
    def enable_compression(
        ...
        self.table = DPTabulate(
            ...
            self.serialize()["activation_function"],
        )
        self.table_config = [
            table_extrapolate,
            table_stride_1,
    ...
    def _pass_filter(
        ...
        if not self.type_one_side and type_embedding is None:
            ...
            if len(self.exclude_types):
                mask = self.emask # question
                # mask = self.build_type_exclude_mask(
                #     self.exclude_types,
                #     self.ntypes,
                #     self.sel, # equal to self.sel_a in tf
                #     self.ndescrpt,
                #     self.atype_nloc,
                #     inputs_i.size(0),
                # )
        ...
        if self.compress and (not is_exclude):
            if self.stripped_type_embedding:
                ...
                return torch.ops.deepmd.tabulate_fusion_se_atten(
                    self.table.data[net].to(dtype=self.prec), # equal to self.filter_precision in tf
                    info,
                    xyz_scatter,
