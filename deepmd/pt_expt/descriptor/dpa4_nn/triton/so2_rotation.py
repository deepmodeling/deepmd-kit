...
from deepmd.dpmodel.descriptor.dpa4_nn.indexing import (
    build_m_major_index as _build_m_major_index_np,
)
...
def build_m_major_index(lmax: int, mmax: int, device: torch.device) -> Tensor:
    """Torch m-major reduced coefficient index on ``device``.

    The dpmodel index builder is numpy-only; the Triton eager-fallback paths
    need it as an int64 tensor on the working device.
    """
    return torch.as_tensor(_build_m_major_index_np(int(lmax), int(mmax)), device=device)
...
_rotate_to_local_op = torch.library.triton_op(
    "dpa4_triton::rotate_to_local", mutates_args=()
)(_rotate_to_local_impl)

_rotate_to_local_bwd_op = torch.library.triton_op(
    "dpa4_triton::rotate_to_local_bwd", mutates_args=()
...
_rotate_back_bwd_op = torch.library.triton_op(
    "dpa4_triton::rotate_back_bwd", mutates_args=()
...
_block_back_so2_op = torch.library.triton_op(
    "dpa4_triton::rotate_back_block_so2", mutates_args=()
)(_block_rotate_back_so2_impl)

_block_back_so2_bwd_op = torch.library.triton_op(
    "dpa4_triton::rotate_back_block_so2_bwd", mutates_args=()
