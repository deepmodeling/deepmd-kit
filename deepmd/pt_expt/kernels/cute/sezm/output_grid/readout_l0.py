...
def has_neo_readout_contract(output_ffn: Any) -> bool:
    """Return whether an output FFN matches the fused Neo readout contract.

    Parameters
    ----------
    output_ffn : Any
        Candidate equivariant output FFN.

    Returns
    -------
    bool
        Whether the FFN and its grid projection expose the required structure.
    """
    if type(output_ffn).__name__ != "EquivariantFFN":
        return False
    grid_net = output_ffn.act
    if (
        type(grid_net).__name__ != "SO3GridNet"
        or type(grid_net.grid_op).__name__ != "GridMLP"
    ):
        return False
...
def _has_exact_neo_readout_contract(
    ...
    if (
        not has_neo_readout_contract(output_ffn)
...
def _project_frames(
    ...
    """Apply one channel projection independently to every frame."""
    n_batch, coefficient_dim, n_focus, _ = coeff.shape
    projected = projection(
        coeff.reshape(n_batch, coefficient_dim, n_focus, n_frames, -1)
    )
    return projected.reshape(n_batch, coefficient_dim, n_focus, -1)
...
def prepare_sm80_readout_input_fold(
    ...
    if not has_neo_readout_contract(output_ffn) or not _inference_mode_is_frozen(
        output_ffn
    ):
...
def _run_neo_readout_l0(
    ...
    if not has_neo_readout_contract(output_ffn):
    ...
    *,
    ...
    if parameters_frozen:
        ...
        if candidate is not None:
            return candidate
    forward_scalar = getattr(output_ffn, "forward_scalar", None)
    if forward_scalar is None:
        forward_scalar = output_ffn.call_scalar
    return (ffn_in[:, 0:1, :, :] + forward_scalar(ffn_in)).reshape(
...
def _readout_l0_impl(
    ...
    from .kernels.readout_l0 import (
        run_readout_l0,
    )
