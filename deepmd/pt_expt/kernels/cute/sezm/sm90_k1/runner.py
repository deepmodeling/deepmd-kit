...
"""Split-complex strict-FP32 SM90 implementation of the complete Neo K1.
...
def _prepare_persistent_weights(so2: Any) -> NeoPersistentComplexWeights:
    ...
    if len(linears) != 3 or len(nonlinearities) != 3:
        raise NotImplementedError("SM90 K1 requires three SO2 layers")
    if any(type(norm).__name__ != "Identity" for norm in so2.so2_inter_norms):
        raise NotImplementedError("SM90 K1 requires disabled inter-layer norms")
    for nonlinearity in nonlinearities[:GATED_LAYERS]:
        if getattr(nonlinearity, "layout", None) != "fndc":
            raise NotImplementedError("SM90 K1 requires fndc gates")
        activation = getattr(
        ...
        if str(activation).lower() != "silu":
            raise NotImplementedError("SM90 K1 requires SiLU gates")
        if getattr(nonlinearity.gate_linear, "bias", None) is not None:
            raise NotImplementedError("SM90 K1 does not support gate bias")
...
def _prepare_final_weights(so2: Any) -> ExpandedFinalWeights:
    ...
    if tuple(w0.shape) != (FOCUS_COUNT, M0_WIDTH, M0_WIDTH):
        raise ValueError("SM90 K1 requires final m=0 weights (2,128,128)")
    if tuple(wp.shape) != (FOCUS_COUNT, 2 * M1_WIDTH, 2 * M1_WIDTH):
        raise ValueError("SM90 K1 requires final pair weights (2,192,192)")
...
class NeoSm90K1Runner(NeoFullCuteBackward):
    """Complete Neo K1 runner with one native split representation."""
    ...
    def _build_forward_graph(self) -> None:
        ...
        if self.compute_capability != (9, 0):
            raise RuntimeError("split-complex K1 requires SM90")
    ...
    def input_adjoint(self, grad_out: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        """Return K1 input adjoints through the split-complex reverse path."""
...
def _final_manual_backward_sm90(
    ...
    """Use the one-slab tiled message-grid adjoint in the final K1 boundary."""
    if not (
        ...
        raise RuntimeError("SM90 K1 requires the packed message-grid path")
