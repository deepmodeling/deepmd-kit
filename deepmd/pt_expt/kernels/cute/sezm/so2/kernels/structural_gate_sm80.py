...
"""Strict-FP32 vectorized structural-gate forward/backward for Neo SO2.
...
def compile_neo_gate_split_structural_vec4_sm80_forward(
    ...
    if (
        ...
        and compile_identity[1:] not in runtime_policy.SUPPORTED_SO2_CAPABILITIES
    ):
        raise ValueError(
            "vectorized structural gate forward requires a supported SO2 device"
        )
    rows = cute.sym_int64()
...
def compile_neo_gate_split_structural_vec4_sm80_backward(
    ...
    if (
        ...
        raise ValueError(
            "vectorized structural gate backward requires a supported SO2 device"
        )
    rows = cute.sym_int64()
    edges = cute.sym_int64()
