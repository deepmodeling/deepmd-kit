...
"""Packed CuTe Wigner-D panel for the Neo SO2 inference path."""
...
from .so2.wigner_layout import PACKED_VALUE_COUNT as SO2_PANEL_VALUES
...
def _wignerd_panel_bwd_impl(
    ...
    q = edge_quat.detach().contiguous()
    if tuple(grad_panel.shape) != (q.shape[0], SO2_PANEL_VALUES):
        raise ValueError(
            f"packed Wigner gradient must have shape ({q.shape[0]}, {SO2_PANEL_VALUES})"
        )
    if q.dtype != torch.float32:
        raise TypeError(f"packed WignerD requires float32, got {q.dtype}")
    if q.shape[0] == 0:
...
_WIGNERD_CUSTOM_OP_TAGS = _stateful_custom_op_tags()
_wignerd_panel_op = torch.library.custom_op(
    "sezm_cute::wignerd_so2_panel", mutates_args=(), tags=_WIGNERD_CUSTOM_OP_TAGS
)(_wignerd_panel_impl)
_wignerd_panel_bwd_op = torch.library.custom_op(
    "sezm_cute::wignerd_so2_panel_bwd", mutates_args=(), tags=_WIGNERD_CUSTOM_OP_TAGS
)(_wignerd_panel_bwd_impl)


@_wignerd_panel_op.register_fake
def _(edge_quat: torch.Tensor) -> torch.Tensor:
    return edge_quat.new_empty((edge_quat.shape[0], SO2_PANEL_VALUES))
...
def _wignerd_panel_backward(
    ...
    """Run packed Wigner-D backward with its custom op visible to compilation."""
...
def run_cute_wignerd(
    ...
    *,
    ...
    """Run packed Wigner-D through its registered custom-op boundary."""
