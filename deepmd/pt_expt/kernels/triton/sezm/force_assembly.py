...
energy, the extended force and per-atom virial are
    ...
    ``W_k = sum_{src(e)=k} ( -g_e (x) edge_vec_e )``.

The reference assembly issues three ``index_add`` scatters (force to both
endpoints and virial to the source) plus a materialized ``(E, 9)``
...
(one over the destination order, one over the source order).  The source pass
recomputes the per-edge outer product on the fly and attributes it in full to
that endpoint, matching the canonical DeePMD atom-virial convention. One
program owns one extended atom; its outputs accumulate in float64 registers,
which removes atomic serialization and tightens the summation error over the
reference fp32 atomics.
...
if FORCE_ASSEMBLY_TRITON_AVAILABLE:
    ...
    def _force_segment_kernel(
        ...
        COMPUTE_VIRIAL: tl.constexpr,  # only the source owns atom virial
    ):
        ...
        for i in range(beg, end):
            ...
            if COMPUTE_VIRIAL:
                ...
                g_k = tl.load(
                    g_ptr + e * 3 + kw // 4, mask=(kw // 4) < 3, other=0.0
                ).to(tl.float64)
                acc_w -= g_k * v_j
        ...
        if COMPUTE_VIRIAL:
            tl.store(
                w_ptr + node * 9 + w_col,
                acc_w.to(w_ptr.dtype.element_ty),
                mask=w_mask,
            )
...
def edge_force_assembly(
    ...
    virial : Tensor
        Extended per-atom virial with shape (N_ext, 9), attributed in full to
        the source endpoint of each edge.
    """
