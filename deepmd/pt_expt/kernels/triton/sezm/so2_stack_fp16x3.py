...
def _mixing_stack_fp16x3_impl(
    ...
    # The level-3 operator is inference-only. The final gated activation is
    # private parameter-gradient state of the fp32 training operator and does
    # not belong to this operator's output contract.
    return x_local, z_all
...
def _mixing_stack_fp16x3_bwd_impl(
    ...
    point_schedule = _point_backward_schedule(focus_dim, lmax)
    ...
    # === Gated layers in reverse; scratch buffers are reused across layers ===
    # Parameter cotangents are outside the inference-only contract. Keeping the
    # scratch as base tensors also gives Inductor functionalization direct
    # mutation targets instead of view-valued output buffers.
...
def _(
    ...
    _, n_focus, n_edge, row = z_all.shape
    return (
