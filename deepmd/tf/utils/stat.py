...
def compute_output_stats(
    ...
    assigned_bias : np.ndarray, optional
        Preset atomic bias, with NaN for unassigned atom types.
    ...
    for key in keys:
        if key in all_stat:
            energy_batches = all_stat[key]
            natoms_key = "real_natoms_vec" if mixed_type else "natoms_vec"
            natoms_batches = all_stat[natoms_key]

            energy_data = []
            natoms_data = []
            for energy_batch, natoms_batch in zip(
                energy_batches, natoms_batches, strict=True
            ):
                energy_batch = np.asarray(energy_batch)
                nframes = energy_batch.shape[0]
                energy_data.append(energy_batch.reshape(nframes, -1))

                natoms_batch = np.asarray(natoms_batch)
                if natoms_batch.ndim == 1:
                    natoms_batch = np.tile(natoms_batch, (nframes, 1))
                natoms_data.append(natoms_batch[:, 2:])

            energy_data = np.concatenate(energy_data, axis=0)
            natoms_data = np.concatenate(natoms_data, axis=0)
