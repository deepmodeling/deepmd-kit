...
def build_neighbor_list(
    ...
    xmax = tf.cond(
        tf.size(coord) > 0,
        lambda: tnp.max(coord) + 2.0 * rcut,
        lambda: tf.cast(2.0 * rcut, coord.dtype),
    )
    # nf x nall
    is_vir = atype < 0
    coord1 = tnp.where(
    ...
    def truncate() -> tuple[tnp.ndarray, tnp.ndarray]:
        return rr[:, :, :nsel], nlist[:, :, :nsel]

    def pad() -> tuple[tnp.ndarray, tnp.ndarray]:
        return (
            tnp.concatenate(
                [
                    rr,
                    tnp.ones([batch_size, nloc, nsel - nnei], dtype=rr.dtype) + rcut,
                ],
                axis=-1,
            ),
            tnp.concatenate(
                [
                    nlist,
                    tnp.ones([batch_size, nloc, nsel - nnei], dtype=nlist.dtype),
                ],
                axis=-1,
            ),
        )

    rr, nlist = tf.cond(nsel <= nnei, truncate, pad)
    nlist = tnp.where(
...
def extend_coord_with_ghosts(
    ...
    def no_pbc() -> tuple[tnp.ndarray, tnp.ndarray, tnp.ndarray]:
        nall = nloc
        extend_coord = coord
        extend_atype = atype
        extend_aidx = aidx
    ...
    def with_pbc() -> tuple[tnp.ndarray, tnp.ndarray, tnp.ndarray]:
        coord_ = tnp.reshape(coord, (nf, nloc, 3))
        cell_ = tnp.reshape(cell, (nf, 3, 3))
        to_face = to_face_distance(cell_)
        ...
        nall = ns * nloc
        shift_vec = tnp.einsum("sd,fdk->fsk", shift_idx, cell_)
        # shift_vec = tnp.tensordot(shift_idx, cell, axes=([1], [1]))
        # shift_vec = tnp.transpose(shift_vec, (1, 0, 2))
        extend_coord = coord_[:, None, :, :] + shift_vec[:, :, None, :]
    ...
    return tf.cond(tf.shape(cell)[-1] == 0, no_pbc, with_pbc)
