...
def descrpt_se_atten_args():
    doc_compressible = "Whether to build a compressible `se_atten` that is supported by `dp compress`. Attention: the compressible `se_atten` disables attention layers. Setting it to `False` will fallback to the previous behavior of `se_atten`."
