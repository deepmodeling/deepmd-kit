...
def validating_args() -> Argument:
    ...
    doc_validation_metric = (
        ...
        "`E` and `V` are per-atom metrics; `F` uses per-atom force-error "
        "vector L2 norms. The corresponding loss prefactors must not both be 0."
    )
    doc_full_val_file = (
...
def validate_full_validation_config(
    ...
    if pref_start == 0.0 and pref_limit == 0.0:
        raise ValueError(
            f"validating.validation_metric={metric!r} requires "
            f"`loss.{pref_start_key}` and `loss.{pref_limit_key}` to not both "
            "be 0."
        )
