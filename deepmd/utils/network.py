...
def share_embedding_network_oneside(xx,
                  atype,
                  nei_type,
                  nei_embed,
                  network_size,
    ...
    xx : Tensor   
        Input tensor of shape [-1,1]
    atype: Tensor
        atom type (length is natom)
    nei_type: Tensor
        The type mask of neighbors
    network_size: list of int
        Size of the embedding network. For example [16,32,64]
    precision: 
        Precision of network weights. For example, tf.float64
    ...
    """
    outputs_size = [1] + network_size
    w = tf.get_variable('matrix_'+str(1)+name_suffix, 
                            [outputs_size[0], outputs_size[1]], 
                            precision,
                            tf.random_normal_initializer(stddev=stddev/np.sqrt(outputs_size[1]+outputs_size[0]), seed = seed), 
                            trainable = trainable)
    variable_summaries(w, 'matrix_'+str(1)+name_suffix)

    b = tf.get_variable('bias_'+str(1)+name_suffix, 
                            [1, outputs_size[1]], 
                            precision,
                            tf.random_normal_initializer(stddev=stddev, mean = bavg, seed = seed), 
                            trainable = trainable)
    variable_summaries(b, 'bias_'+str(1)+name_suffix)

    hidden = tf.reshape(activation_fn(tf.matmul(xx, w) + b), [-1, outputs_size[1]]) # atom * nnei , output_size[1]

    xx = hidden + nei_embed + atm_embed
    """
    
    
    outputs_size =  network_size
    xx = tf.concat([xx,nei_embed],1)
...
def share_embedding_network_twoside(xx,
                  atype,
                  nei_type,
                  nei_embed,
                  atm_embed,
    ...
    outputs_size =  network_size
    xx = tf.concat([xx,nei_embed,atm_embed],1)
