...
def compute_stats_from_atomic(
    ...
    for type_i in range(ntypes):
        mask = atype == type_i
        output_bias[type_i] = output[mask].mean(axis=0) if output[mask].size > 0 else np.nan
        output_std[type_i] = output[mask].std(axis=0) if output[mask].size > 0 else np.nan
