...
```bash
cd /some/workspace
wget https://github.com/lammps/lammps/archive/stable_29Aug2024_update2.tar.gz
tar xf stable_29Aug2024_update2.tar.gz
```

The source code of LAMMPS is stored in the directory `lammps-stable_29Aug2024_update2`.
...
```bash
cd lammps-stable_29Aug2024_update2/src/
...
```bash
mkdir -p lammps-stable_29Aug2024_update2/build/
cd lammps-stable_29Aug2024_update2/build/
```
...
The source code of LAMMPS is stored in the directory `lammps-stable_29Aug2024_update2`. The directory of the source code should be specified as the CMAKE argument `LAMMPS_SOURCE_ROOT` during installation of the DeePMD-kit C++ interface. Now go into the LAMMPS directory and create a directory called `build`
