...
### Creating atomic_dipole Data

The `atomic_dipole.npy` file contains atomic dipole vectors for each atom in each frame. There are several ways to create this data:

#### 4.13.4.1. From Wannier Centroids (for DPLR models)

For Deep Potential Long Range (DPLR) models, the atomic dipole represents the vector pointing from each atom to the center of its electronic density (Wannier centroid). This data can be obtained from first-principles calculations:

1. **Using VASP + Wannier90**: Run DFT calculations with VASP and use Wannier90 to compute maximally localized Wannier functions. The Wannier centroids can then be converted to atomic dipoles.

2. **Data format**: The atomic dipole should be a 3D vector for each atom, representing the displacement from the atom position to its associated Wannier centroid.
...
3. **Example workflow**:
   ...
   # Register atomic_dipole data type with dpdata
   datatype = DataType(
       "atomic_dipole",
       np.ndarray,
   ...
   # Load your system data
   system = dpdata.LabeledSystem("your_data_directory")

   # Add atomic dipole data (replace with your calculated values)
   # atomic_dipoles shape: (n_frames, n_atoms, 3)
   system.data["atomic_dipole"] = your_atomic_dipole_array

   # Save in DeePMD format
   system.to_deepmd_npy("output_directory")
   ```

#### 4.13.4.2. From Raw Text Files

If you have atomic dipole data in text format, you can convert it using the standard raw-to-npy conversion:

1. **Create atomic_dipole.raw**: Format your data as a text file where each row represents one frame, and each column represents the x, y, z components of atomic dipoles for all atoms:

   ```
   # Frame 1: atom1_x atom1_y atom1_z atom2_x atom2_y atom2_z ...
   0.123 -0.456 0.789 0.234 -0.567 0.891 ...
   # Frame 2:
   0.135 -0.468 0.801 0.246 -0.579 0.903 ...
   ```

2. **Convert to numpy format**: Use the conversion script provided in DeePMD-kit:
   ```bash
   cd your_dataset/set.000/
   # Place your atomic_dipole.raw file here
   python -c "
   import numpy as np
...
#### 4.13.4.3. Data Shape Requirements

The `atomic_dipole.npy` file must have the shape `(n_frames, n_atoms * 3)`, where:

- `n_frames`: Number of molecular configurations
- `n_atoms`: Number of atoms per frame
- `3`: x, y, z components of the dipole vector

For example, with 10 frames and 32 atoms per frame, the shape should be `(10, 96)`.

#### 4.13.4.4. Data Units and Conventions

- **Units**: The atomic dipole data can use any consistent unit (often Å×e where e is the elementary charge)
- **Reference**: Atomic dipoles are typically relative to the atomic positions
- **Zero dipoles**: For atoms that don't contribute to the property being modeled (e.g., hydrogen atoms in some dipole models), the corresponding dipole components should be set to zero

#### 4.13.4.5. Hands-on Tutorial

For a practical example of creating DPLR models with Wannier centroids, see the hands-on tutorial: [CSI Princeton Workshop - Deep Wannier/DPLR](https://github.com/CSIprinceton/workshop-june-2024/tree/main/hands-on-sessions/day-2-DW-DPLR/7-deep-wannier).
