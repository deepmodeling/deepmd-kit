...
Thin wrapper over :class:`dpa_adapt.data.assemblies.AssemblyDatasetBuilder`:
...
class PolymerBuilder:
    ...
    def write(
        ...
        *,
        ...
        """
        from dpa_adapt.data.assemblies import AssemblyDatasetBuilder, ComponentSpec
        ...
        builder = AssemblyDatasetBuilder(property_name=self.target, type_map=type_map)
        for row in self._rows:
            cond_vec = self._standardized_conditions(row, schema, stats)
            group = builder.group(key=row.key, label=row.target, conditions=cond_vec)
        ...
        result = builder.write_deepmd_npy(out, overwrite=overwrite)
