...
def _cmd_cv(args: argparse.Namespace) -> int:
    from dpa_tools import DPAFineTuner, cross_validate, load_dataset
...
def _cmd_data_convert(args: argparse.Namespace) -> int:
    ...
    if any(ch in input_val for ch in "*?["):
        from dpa_tools import batch_convert
    ...
    # Single-file mode.
    from dpa_tools.data.convert import auto_convert
...
def _cmd_data_validate(args: argparse.Namespace) -> int:
    from dpa_tools import check_data
...
def _cmd_data_attach_labels(args: argparse.Namespace) -> int:
    from dpa_tools import attach_labels
    from dpa_tools.data.loader import load_data
