...
from dpa_tools.data.validate import check_data
...
def auto_convert(
    ...
    *If ``fmt="formula"``* the call delegates to
    :func:`~dpa_tools.data.formula.formula_to_npy`, which reads a
    ...
    *If the input is a CSV / Excel file with SMILES columns* the call
    delegates to :func:`~dpa_tools.data.smiles.smiles_to_npy`, which
    ...
    *Otherwise* the call delegates to ``dpdata`` with ``fmt="auto"`` (or the
    ...
    if is_smiles_fmt or (fmt is None and _is_smiles_input(input_path)):
        from dpa_tools.data.smiles import smiles_to_npy
