...
class TestInferDeepPotDefaultFParam : public ::testing::Test {
 protected:
...
TYPED_TEST_SUITE(TestInferDeepPotDefaultFParam, ValueTypes);

TYPED_TEST(TestInferDeepPotDefaultFParam, attrs) {
...
TYPED_TEST(TestInferDeepPotDefaultFParam, cpu_build_nlist_empty_fparam) {
  using VALUETYPE = TypeParam;
...
TYPED_TEST(TestInferDeepPotDefaultFParam, cpu_build_nlist_explicit_fparam) {
  using VALUETYPE = TypeParam;
  std::vector<VALUETYPE>& coord = this->coord;
  std::vector<int>& atype = this->atype;
  std::vector<VALUETYPE>& box = this->box;
