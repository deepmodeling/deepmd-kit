...
class TestInferDeepSpinTFAHPP : public ::testing::Test {
 protected:
  ...
  void TearDown() override { remove("deepspin_nlist_hpp.pb"); };
};

TYPED_TEST_SUITE(TestInferDeepSpinTFAHPP, ValueTypes);

TYPED_TEST(TestInferDeepSpinTFAHPP, cpu_build_nlist) {
...
template <class VALUETYPE>
class TestInferDeepSpinTFANoPbcHPP : public ::testing::Test {
 protected:
  std::vector<VALUETYPE> coord = {12.83, 2.56, 2.18, 12.09, 2.87, 2.74,
                                  3.51,  2.51, 2.60, 4.27,  3.22, 1.56};
  std::vector<VALUETYPE> spin = {0., 0., 1.2737, 0., 0., 1.2737,
  ...
  void SetUp() override {
    std::string file_name = "../../tests/infer/deepspin_nlist.pbtxt";
    deepmd::hpp::convert_pbtxt_to_pb(file_name, "deepspin_nlist_hpp.pb");
...
TYPED_TEST_SUITE(TestInferDeepSpinTFANoPbcHPP, ValueTypes);

TYPED_TEST(TestInferDeepSpinTFANoPbcHPP, cpu_build_nlist) {
  using VALUETYPE = TypeParam;
  std::vector<VALUETYPE>& coord = this->coord;
  std::vector<VALUETYPE>& spin = this->spin;
  std::vector<int>& atype = this->atype;
...
TYPED_TEST(TestInferDeepSpinTFANoPbcHPP, cpu_lmp_nlist) {
  using VALUETYPE = TypeParam;
  std::vector<VALUETYPE>& coord = this->coord;
  std::vector<VALUETYPE>& spin = this->spin;
  std::vector<int>& atype = this->atype;
