...
inline TFE_Op* get_func_op(TFE_Context* ctx,
  ...
  if (func == NULL) {
    // note: the message format is used by error catching of do_message_passing
    throw deepmd::deepmd_exception("Function " + func_name + " not found");
  }
...
void deepmd::DeepPotJAX::init(const std::string& model,
  ...
  try {
    ...
    if (e.what() == "Function do_message_passing not found") {
