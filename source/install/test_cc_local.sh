...
'
	...
	# prevents a partial/stale cache from silently turning missing coverage into
	# GTEST_SKIP or making the C++ tests consume mismatched reference values.
	if [ "${DP_REUSE_GENERATED_PT_MODELS:-0}" = "1" ]; then
		...
		# Remove stale generated files so they can't be accidentally reused if
		# generation scripts or formats change.  *.expected sidecars are consumed
		# by the C++ tests and therefore belong to the same cache unit as PT2/PTE.
		...
		# Record the exact generated fixture set.  A warm cache is accepted only
		# when every entry in this manifest was restored.
