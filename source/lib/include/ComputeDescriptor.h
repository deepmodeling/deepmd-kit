...
inline int format_nlist_fill_a(vector<int> &fmt_nei_idx_a,
...
inline void compute_descriptor(vector<double> &descrpt_a,
...
inline void compute_descriptor_se_a(vector<double> &descrpt_a,
                                    vector<double> &descrpt_a_deriv,
                                    vector<double> &rij_a,
                                    const vector<double> &posi,
                                    const int &ntypes,
...
inline void compute_descriptor_se_r(vector<double> &descrpt_r,
                                    vector<double> &descrpt_r_deriv,
                                    vector<double> &rij_r,
                                    const vector<double> &posi,
                                    const int &ntypes,
             ...
             const double *rot)
{
  ...
  for (int ii = 0; ii < 3; ++ii)
  {
    for (int jj = 0; jj < 3; ++jj)
    {
      dRdy[ii * 3 + jj] = (-2 * r1dr2 / nr14 * r1[ii] * r1[jj] + (r1[ii] + r2[ii]) * r1[jj] / nr12);
      if (ii == jj)
      {
  ...
  for (int ii = 0; ii < 3; ++ii)
  {
    for (int jj = 0; jj < 3; ++jj)
    {
      dRdT1[ii * 3 + jj] = (-ydRdy[ii] * tmpy[jj] / (ntmpy * ntmpy * ntmpy) + dRdy[3 * ii + jj] / ntmpy);
    }
  }
  // dRdT2
  for (int ii = 0; ii < 3; ++ii)
               ...
               const double *rot)
{
  ...
  for (int ii = 0; ii < 3; ++ii)
  {
    for (int jj = 0; jj < 3; ++jj)
    {
      dRdy[ii * 3 + jj] = (+2 * r1dr2 / nr14 * r1[ii] * r1[jj] - r2[ii] * r1[jj] / nr12);
      if (ii == jj)
      {
