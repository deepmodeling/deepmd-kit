...
#define cudaErrcheck(res)                  \
  {                                        \
    cudaAssert((res), __FILE__, __LINE__); \
  }
...
void NNPInter::
    compute_inner(ENERGYTYPE &dener,
                  ...
                  const vector<VALUETYPE> &aparam)
{
  cout << " in NNPInter::compute_inner 1 ------------------------\n";
