...
__global__ void get_i_idx_se_a(
    int * i_idx,
    const int nloc,
    const int * ilist)
{
...
{ 
  ...
  int_64 * key_out = key + nloc * MAX_NBOR_SIZE + idx * MAX_NBOR_SIZE;
...
{   
  ...
  format_nlist_fill_a_se_a<<<block_grid, thread_grid>>> (
      key,
    ...
    const std::vector<int> sec)
{
  ...
  cudaErrcheck(cudaMemset(key, 0xffffffff, sizeof(int_64) * nloc * GPU_MAX_NBOR_SIZE));
...
template void prod_env_mat_a_gpu_nv<float>(float * em, float * em_deriv, float * rij, int * nlist, const float * coord, const int * type, const int * ilist, const int * jrange, const int * jlist, int * array_int, unsigned long long * array_longlong, const int max_nbor_size, const float * avg, const float * std, const int nloc, const int nall, const int ntypes, const float rcut, const float rcut_smth, const std::vector<int> sec);
template void prod_env_mat_a_gpu_nv<double>(double * em, double * em_deriv, double * rij, int * nlist, const double * coord, const int * type, const int * ilist, const int * jrange, const int * jlist, int * array_int, unsigned long long * array_longlong, const int max_nbor_size, const double * avg, const double * std, const int nloc, const int nall, const int ntypes, const float rcut, const float rcut_smth, const std::vector<int> sec);