...
template <typename FPTYPE, int MTILE, int KTILE>
__global__ void tabulate_fusion_se_atten_fifth_order_polynomial(
    FPTYPE* out,
    const FPTYPE* table,
    const FPTYPE* em_x,
    const FPTYPE* em,
...
template <typename FPTYPE, int MTILE, int KTILE>
__global__ void tabulate_fusion_se_atten_grad_fifth_order_polynomial(
    FPTYPE* dy_dem_x,
    FPTYPE* dy_dem,
    const FPTYPE* table,
    const FPTYPE* em_x,
    ...
    for (int jj = lane_idx; jj < last_layer_size; jj += WARP_SIZE) {
      ...
      FPTYPE res =
          ...
          (var[1] +
           (var[2] + (var[3] + (var[4] + var[5] * xx) * xx) * xx) * xx) *
              xx;
      FPTYPE t = two_embed[block_idx * nnei*last_layer_size + ii * last_layer_size + jj];
      res = res * t + t;
...
template <typename FPTYPE, int MTILE, int KTILE>
__global__ void tabulate_fusion_se_atten_grad_grad_fifth_order_polynomial(
    FPTYPE* dz_dy,
    const FPTYPE* table,
    const FPTYPE* em_x,
    const FPTYPE* em,
...
void tabulate_fusion_se_atten_gpu_cuda(FPTYPE* out,
  ...
  tabulate_fusion_se_atten_fifth_order_polynomial<FPTYPE, MM, KK>
      <<<nloc, last_layer_size>>>(out, table, em_x, em, two_embed, table_info[0],
                                  table_info[1], table_info[2], table_info[3],
                                  table_info[4], nnei, last_layer_size);
  DPErrcheck(cudaGetLastError());
  DPErrcheck(cudaDeviceSynchronize());
...
void tabulate_fusion_se_atten_grad_grad_gpu_cuda(FPTYPE* dz_dy,
  ...
  DPErrcheck(cudaMemset(dz_dy, 0, sizeof(FPTYPE) * nloc * 4 * last_layer_size));
  tabulate_fusion_se_atten_grad_grad_fifth_order_polynomial<FPTYPE, MM, KK>
      <<<nloc, last_layer_size, sizeof(FPTYPE) * MM * last_layer_size>>>(
...
template void tabulate_fusion_se_atten_gpu_cuda<float>(float* out,
                                                   const float* table,
                                                   const float* table_info,
                                                   const float* em_x,
                                                   const float* em,
                                                   ...
                                                   const int nnei,
                                                   const int last_layer_size);
template void tabulate_fusion_se_atten_gpu_cuda<double>(double* out,
                                                    const double* table,
                                                    const double* table_info,
                                                    const double* em_x,
                                                    const double* em,
                                                    ...
                                                    const int nnei,
                                                    const int last_layer_size);
template void tabulate_fusion_se_atten_grad_gpu_cuda<float>(
    float* dy_dem_x,
    float* dy_dem,
    const float* table,
    const float* table_info,
    ...
    const int nnei,
    const int last_layer_size);
template void tabulate_fusion_se_atten_grad_gpu_cuda<double>(
    double* dy_dem_x,
    double* dy_dem,
    const double* table,
    const double* table_info,
    ...
    const int nnei,
    const int last_layer_size);
template void tabulate_fusion_se_atten_grad_grad_gpu_cuda<float>(
    float* dz_dy,
    const float* table,
    const float* table_info,
    const float* em_x,
    ...
    const int nnei,
    const int last_layer_size);
template void tabulate_fusion_se_atten_grad_grad_gpu_cuda<double>(
    double* dz_dy,
    const double* table,
    const double* table_info,
    const double* em_x,
