...
#pragma omp parallel for num_threads(nthreads)
  ...
  // 	int mc = shift0 + shift1 + mm2 + KK[2]/2;
	...
	// virial
	VALUETYPE vpref = (FPTYPE)-2. * ((FPTYPE)1. + M_PI * M_PI * nmm2 / (param.beta * param.beta)) / nmm2;
	...
	// force
	for (int ii = 0; ii < natoms; ++ii){
	  VALUETYPE mdotr = (FPTYPE)-2. * M_PI * (coord[ii*3+0]*rm[0] + coord[ii*3+1]*rm[1] + coord[ii*3+2]*rm[2]);
	  VALUETYPE tmpr = charge[ii] * cos(mdotr);
	  VALUETYPE tmpi = charge[ii] * sin(mdotr);
	  VALUETYPE cc = (FPTYPE)4. * M_PI * (tmpr * sqi[mc] + tmpi * sqr[mc]) * expnmm2;
