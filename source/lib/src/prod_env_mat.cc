    ...
    const std::vector<int> sec) 
{
  const int nnei = sec.back();
  const int nem = nnei * 4;
  memset(em, 0.0, sizeof(FPTYPE) * nloc * nem);
  memset(em_deriv, 0.0, sizeof(FPTYPE) * nloc * nem * 3);
  memset(rij, 0.0, sizeof(FPTYPE) * nloc * nnei * 3);
...
#pragma omp parallel for 
  for (int ii = 0; ii < nloc; ++ii) {
    ...
    // record outputs
    for (int jj = 0; jj < nnei; ++jj) {
      int temp_nei = fmt_nlist_a[jj];
      nlist[ii * nnei + jj] = temp_nei;
      if(temp_nei >= 0){
        for (int kk = 0; kk < 3; ++kk){
          rij[ii * nnei * 3 + jj*3 + kk] = d_rij_a[jj*3 + kk];
        }
        for (int kk = 0; kk < 4; ++kk) {
          em[ii * nem + jj * 4 + kk] = (d_em_a[jj * 4 + kk] - avg[d_type[ii] * nem + jj * 4 + kk]) / std[d_type[ii] * nem + jj * 4 + kk];
        }
        for (int kk = 0; kk < 12; ++kk) {
          em_deriv[ii * nem * 3 + jj * 4 * 3 + kk] = d_em_a_deriv[jj * 4 * 3 + kk] / std[d_type[ii] * nem + jj * 4 + kk / 3];
        }
      }
    }
  //   for (int jj = 0; jj < nem; ++jj) {
