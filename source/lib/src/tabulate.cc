...
template <typename FPTYPE>
void deepmd::tabulate_fusion_se_atten_cpu(FPTYPE* out,
                                          const FPTYPE* table,
                                          const FPTYPE* table_info,
                                          const FPTYPE* em_x,
                                          const FPTYPE* em,
...
template <typename FPTYPE>
void deepmd::tabulate_fusion_se_atten_grad_cpu(FPTYPE* dy_dem_x,
                                               FPTYPE* dy_dem,
                                               const FPTYPE* table,
                                               const FPTYPE* table_info,
                                               const FPTYPE* em_x,
  ...
  for (int ii = 0; ii < nloc; ii++) {
    ...
    for (int jj = 0; jj < nnei; jj++) {
      ...
      for (int kk = 0; kk < last_layer_size; kk++) {
        ...
        if (unloop) {
          grad += deriv * dot(ll, rr) * (nnei - jj);
          dy_dem[ii * nnei * 4 + jj * 4 + 0] += res1 * rr[0] * (nnei - jj);
          dy_dem[ii * nnei * 4 + jj * 4 + 1] += res1 * rr[1] * (nnei - jj);
          dy_dem[ii * nnei * 4 + jj * 4 + 2] += res1 * rr[2] * (nnei - jj);
          dy_dem[ii * nnei * 4 + jj * 4 + 3] += res1 * rr[3] * (nnei - jj);
        } else {
          grad += deriv * dot(ll, rr);
          dy_dem[ii * nnei * 4 + jj * 4 + 0] += res1 * rr[0];
          dy_dem[ii * nnei * 4 + jj * 4 + 1] += res1 * rr[1];
          dy_dem[ii * nnei * 4 + jj * 4 + 2] += res1 * rr[2];
          dy_dem[ii * nnei * 4 + jj * 4 + 3] += res1 * rr[3];
        }
      }
      dy_dem_x[ii * nnei + jj] = grad;
      if (unloop) break;
...
template <typename FPTYPE>
void deepmd::tabulate_fusion_se_atten_grad_grad_cpu(FPTYPE* dz_dy,
                                                    const FPTYPE* table,
                                                    const FPTYPE* table_info,
                                                    const FPTYPE* em_x,
                                                    const FPTYPE* em,
...
template void deepmd::tabulate_fusion_se_atten_cpu<float>(
    float* out,
    const float* table,
    const float* table_info,
    const float* em_x,
    ...
    const int nnei,
    const int last_layer_size);
template void deepmd::tabulate_fusion_se_atten_cpu<double>(
    double* out,
    const double* table,
    const double* table_info,
    const double* em_x,
    ...
    const int nnei,
    const int last_layer_size);
template void deepmd::tabulate_fusion_se_atten_grad_cpu<float>(
    float* dy_dem_x,
    float* dy_dem,
    const float* table,
    const float* table_info,
    ...
    const int nnei,
    const int last_layer_size);
template void deepmd::tabulate_fusion_se_atten_grad_cpu<double>(
    double* dy_dem_x,
    double* dy_dem,
    const double* table,
    const double* table_info,
    ...
    const int nnei,
    const int last_layer_size);
template void deepmd::tabulate_fusion_se_atten_grad_grad_cpu<float>(
    float* dz_dy,
    const float* table,
    const float* table_info,
    const float* em_x,
    ...
    const int nnei,
    const int last_layer_size);
template void deepmd::tabulate_fusion_se_atten_grad_grad_cpu<double>(
    double* dz_dy,
    const double* table,
    const double* table_info,
    const double* em_x,
