...
TEST_F(TestEnvMatA, prod_cpu_multiple_frames) {
  EXPECT_EQ(nlist_r_cpy.size(), nloc);
...
TEST_F(TestEnvMatA, prod_gpu_multiple_frames) {
  EXPECT_EQ(nlist_r_cpy.size(), nloc);
  constexpr int nframes = 2;
  int max_nbor_size = 0;
  for (int ii = 0; ii < nlist_a_cpy.size(); ++ii) {
  ...
  deepmd::prod_env_mat_a_gpu(em_dev, em_deriv_dev, rij_dev, nlist_dev, posi_dev,
