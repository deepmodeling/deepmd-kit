...
TEST_F(TestEnvMatR, prod_cpu_negative_center_type) {
  EXPECT_EQ(nlist_r_cpy.size(), nloc);
  int max_nbor_size = 0;
  for (int ii = 0; ii < nlist_a_cpy.size(); ++ii) {
    if (nlist_a_cpy[ii].size() > max_nbor_size) {
  ...
  std::vector<int> atype_neg = atype_cpy;
  atype_neg[0] = -1;
  std::vector<double> em(static_cast<size_t>(nloc) * ndescrpt, -1.0),
      em_deriv(static_cast<size_t>(nloc) * ndescrpt * 3, -1.0),
  ...
  deepmd::prod_env_mat_r_cpu(em.data(), em_deriv.data(), rij.data(),
                             nlist.data(), posi_cpy.data(), atype_neg.data(),
                             inlist, max_nbor_size, avg.data(), std.data(),
                             nloc, nall, rc, rc_smth, sec_a);

  for (int jj = 0; jj < ndescrpt; ++jj) {
    EXPECT_EQ(em[jj], 0.0);
  }
  for (int jj = 0; jj < ndescrpt * 3; ++jj) {
    EXPECT_EQ(em_deriv[jj], 0.0);
  }
}
...
TEST_F(TestEnvMatR, prod_cpu_multiple_frames) {
  EXPECT_EQ(nlist_r_cpy.size(), nloc);
  ...
  for (int ff = 0; ff < nframes; ++ff) {
    ...
    for (size_t jj = 0; jj < frame_em.size(); ++jj) {
      EXPECT_LT(fabs(em[em_offset + jj] - frame_em[jj]), 1e-10);
    }
    for (size_t jj = 0; jj < frame_em_deriv.size(); ++jj) {
      EXPECT_LT(fabs(em_deriv[deriv_offset + jj] - frame_em_deriv[jj]), 1e-10);
    }
    for (size_t jj = 0; jj < frame_rij.size(); ++jj) {
      EXPECT_LT(fabs(rij[rij_offset + jj] - frame_rij[jj]), 1e-10);
    }
    for (size_t jj = 0; jj < frame_nlist.size(); ++jj) {
      EXPECT_EQ(nlist[nlist_offset + jj], frame_nlist[jj]);
    }
  }
}
