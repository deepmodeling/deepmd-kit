...
protected:
  ...
  void SetUp() override {
	...
	nlist[ii*nnei + jj] = fmt_nlist_a[jj];
      ...
      env_mat_a_cpu<double>(t_env, t_env_deriv, t_rij_a, posi_cpy, ntypes, atype_cpy, ii, fmt_nlist_a, sec_a, rc_smth, rc);    
...
TEST_F(TestProdForceGradA, cpu)
{
  ...
  prod_force_grad_a_cpu<double>(&grad_net[0], &grad[0], &env_deriv[0], &nlist[0], nloc, nnei);
