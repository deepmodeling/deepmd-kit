  ...
  output_file(nullptr)
{
  ...
  if (narg < 4) error->all(FLERR,"Illegal fix cboamd command");
  
  // Parse command line arguments
  int iarg = 3;
  while (iarg < narg) {
    if (strcmp(arg[iarg], "dipole") == 0) {
      if (iarg + 1 >= narg) error->all(FLERR,"Illegal fix cboamd command");
      ...
      if (iarg + 1 + nphoton > narg) error->all(FLERR,"Illegal fix cboamd command");
      ...
      if (iarg + 1 + 3 * nphoton > narg) error->all(FLERR,"Illegal fix cboamd command");
      memory->create(lambda_vector, nphoton, 3, "fix_cboamd:lambda_vector");
      for (int i = 0; i < nphoton; i++) {
        for (int j = 0; j < 3; j++) {
          lambda_vector[i][j] = utils::numeric(FLERR, arg[iarg + 1 + 3 * i + j], false, lmp);
        }
      }
      iarg += 1 + 3 * nphoton;
    } else {
  ...
  if (!model_dipole) error->all(FLERR,"fix cboamd requires dipole model");
  // if (!model_polar) error->all(FLERR,"fix cboamd requires polarizability model");
  if (photons_enabled && nphoton <= 0) error->all(FLERR,"fix cboamd: nphoton must be > 0 when photons enabled");
  if (photons_enabled && !omega_photon) error->all(FLERR,"fix cboamd: omega must be specified when photons enabled");
  if (photons_enabled && !lambda_photon) error->all(FLERR,"fix cboamd: lambda must be specified when photons enabled");
  if (photons_enabled && !lambda_vector) error->all(FLERR,"fix cboamd: lambda_vector must be specified when photons enabled");
  ...
  if (!output_file) error->all(FLERR,"Cannot open cboamd output file");
  fprintf(output_file, "# Step Time Energy Dipole_x Dipole_y Dipole_z Pol_xx Pol_xy Pol_xz Pol_yx Pol_yy Pol_yz Pol_zx Pol_zy Pol_zz");
  if (photons_enabled) {
    for (int i = 0; i < nphoton; i++) {
...
FixCBOAMD::~FixCBOAMD()
{
  ...
  if (model_potential) delete[] model_potential;
  if (model_dipole) delete[] model_dipole;
  if (model_polar) delete[] model_polar;
...
int FixCBOAMD::size_restart(int nlocal)
{
  return 3;
}
...
void FixCBOAMD::init_deepmd_models()
{
  try {
    ...
    if (model_polar) deepmd_polar = new deepmd_compat::DeepTensor(model_polar);
    
    if (comm->me == 0) {
      utils::logmesg(lmp, "DeepMD models initialized successfully:\n");
      // utils::logmesg(lmp, "  Potential: {}\n", model_potential);
      utils::logmesg(lmp, "  Dipole: {}\n", model_dipole);
      if (model_polar) utils::logmesg(lmp, "  Polarizability: {}\n", model_polar);
    }
...
void FixCBOAMD::compute_deepmd_dipole()
{
  try {
    ...
    // dipole_grad_deepmd is the negative gradient of dipole w.r.t. atomic coordinates
    // It is a flattened array of size 3 (dipole components) * N (atoms) * 3 (atomic coordinate components)
    deepmd_dipole->compute(dipole_deepmd, dipole_grad_deepmd, 
      dipole_virial_deepmd, dipole_atom_deepmd, dipole_atom_virial_deepmd,  
...
void FixCBOAMD::compute_deepmd_polarizability()
{
  try {
    ...
    deepmd_polar->compute(polar_deepmd, polar_grad_deepmd, polar_virial_deepmd, 
      polar_atom_deepmd, polar_atom_virial_deepmd, coords_deepmd, atom_types_deepmd, cell_deepmd);
    
    // Extract polarizability components (DeepMD returns in eV/A, convert to a.u.)
    for (int i = 0; i < 9; i++) {
...
void FixCBOAMD::compute_cboa_forces()
{
  ...
  for (int dp = 0; dp < 3; dp++) { // dp is the direction of dipole or photon
    for (int i = 0; i < nlocal; i++) {
      for (int di = 0; di < 3; di++) { // di is the direction of atomic coordinate
...
void FixCBOAMD::write_output()
{
  ...
  if (comm->me == 0) {
    ...
    if (photons_enabled) {
      for (int i = 0; i < nphoton; i++) {
        fprintf(output_file, " %.6e %.6e %.6e %.6e", qa[i], pa[i], ea[i], fa[i]);
      }
    }
    fprintf(output_file, "\n");
    fflush(output_file);
...
void FixCBOAMD::convert_coordinates_to_deepmd_format()
{
  ...
  cell_deepmd[0] = lx; cell_deepmd[1] = 0.0; cell_deepmd[2] = 0.0;
  cell_deepmd[3] = 0.0; cell_deepmd[4] = ly; cell_deepmd[5] = 0.0;
  cell_deepmd[6] = 0.0; cell_deepmd[7] = 0.0; cell_deepmd[8] = lz;
}
