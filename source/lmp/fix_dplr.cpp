      ...
      efield_force_flag(0)
{
  ...
  while (iarg < narg)
  {
    ...
    {
      if (iarg + 3 > narg)
        error->all(FLERR, "Illegal fix adapt command, efield should be provided 3 float numbers");
...
void FixDPLR::pre_force(int vflag)
{
  ...
  cout << "in Fix DPLR before force---------------------------------" << endl;
...
void FixDPLR::post_force(int vflag)
{
  ...
  cout << "in Fix DPLR atfter force---------------------------------" << endl;
