...
void PairNNP::print_summary(const string pre) const
{
  if (comm->me == 0)
  {
  ...
  if (do_ghost)
  {
    ...
    if (single_model || multi_models_no_mod_devi)
    {
      if (!(eflag_atom || vflag_atom))
      {
        ...
        for (unsigned dd = 0; dd < dbox.size(); ++dd)
          dbox_[dd] = dbox[dd];
...
//         }
    ...
    {
      error->all(FLERR, "not support multi_models_mod_devi");
...
//         for (int dd = 0; dd < std_f_.size(); ++dd)
//           std_f[dd] = std_f_[dd];
//         if (out_rel == 1)
//         {
...
//         for (int dd = 0; dd < std_e_.size(); ++dd)
//           std_e[dd] = std_e_[dd];
// #endif
//         max = avg = 0;
...
//       }
  ...
  {
    error->all(FLERR, "not support no ghost");
//     if (numb_models == 1)
//     {
// #ifdef HIGH_PREC
...
void PairNNP::settings(int narg, char **arg)
{
  ...
  {
    assert(false), "nnp_inter_model_devi not support !!!";
    // nnp_inter.init(arg[0], get_node_rank());
