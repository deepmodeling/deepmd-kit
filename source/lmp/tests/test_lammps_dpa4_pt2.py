...
``has_message_passing=True`` (GNN). The DENSE (nlist) lower has no
comm_dict implementation (``dense_lower_supports_comm() == False``), so
``deserialize_to_file`` never emits a nested with-comm artifact for this
dense archive; multi-rank inference on it fails fast in C++ instead (see
the "Deferred" note below). This is scoped to the dense lower only: the
GRAPH ``.pt2`` (``deeppot_dpa4_graph.pt2``, ``test_lammps_dpa4_graph_pt2.py``)
DOES carry a with-comm artifact (``has_message_passing_across_ranks()``
returns ``self.bridging_switch is None``) and runs multi-rank via the same
generic dispatch DPA2 uses.
...
Deferred (NOT covered): live multi-rank parity for this DENSE archive.
``deeppot_dpa4.pt2`` carries no with-comm artifact, so multi-rank LAMMPS
inference on it fails fast at the first force evaluation instead of
running (see the module-level note); there is no with-comm dispatch to
exercise here, and no mpi runner script. Multi-rank inference for DPA4 IS
covered, but only through the GRAPH archive -- see
``test_lammps_dpa4_graph_pt2.py``'s multi-rank section.
