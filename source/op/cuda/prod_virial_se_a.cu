...
template<typename T>
__global__ void deriv_wrt_neighbors_se_a(
    T * virial, 
    T * atom_virial,
    const T * net_deriv,
    const T * in_deriv,
...
void ProdVirialSeAGPUExecuteLauncher(
    float * virial, 
    float * atom_virial,
    ...
    const int n_a_shift) 
{
    ...
    deriv_wrt_neighbors_se_a <<<block_grid, thread_grid>>>(virial, atom_virial, net_deriv, in_deriv, rij, nlist, nloc, nnei, ndescrpt, n_a_sel, n_a_shift);
}
