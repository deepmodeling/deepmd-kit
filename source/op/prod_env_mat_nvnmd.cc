...
#pragma omp parallel for 
  for (int ii = 0; ii < nloc; ++ii) {
    ...
    env_mat_a_nvnmd_cpu (d_em_a, d_em_a_deriv, d_rij_a, d_coord3, d_type, ii, fmt_nlist_a, sec, rcut_smth, rcut, precs);
...
void 
deepmd::
prod_env_mat_a_nvnmd_cpu<double>(
    double * em, 
    double * em_deriv, 
    double * rij, 
    int * nlist, 
...
void
deepmd::
prod_env_mat_a_nvnmd_cpu<float>(
    float * em, 
    float * em_deriv, 
    float * rij, 
    int * nlist, 
