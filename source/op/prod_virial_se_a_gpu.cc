...
struct NatomsFunctor {
    void operator()(const CPUDevice& d, const int * natoms_ptr, std::vector<int>& natoms) {
        natoms[0] = natoms_ptr[0];
        natoms[1] = natoms_ptr[1];
    }
    #if GOOGLE_CUDA
    void operator()(const GPUDevice& d, const int * natoms_ptr, std::vector<int>& natoms) {
        cudaErrcheck(cudaMemcpy(&natoms[0], natoms_ptr, sizeof(int) * 2, cudaMemcpyDeviceToHost));
    }
    #endif // GOOGLE_CUDA
};
