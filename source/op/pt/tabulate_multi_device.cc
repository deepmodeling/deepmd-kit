...
void TabulateFusionSeAForward(
  ...
  if(two_embed_tensor == nullptr || two_embed_tensor.dim() != 2)
    throw std::invalid_argument("Dim of input should be 2");
  // get the device
  std::string device;
  ...
  const FPTYPE* two_embed = (two_embed_tensor == nullptr) ? nullptr : two_embed_tensor.view({-1}).data_ptr<FPTYPE>();
...
void TabulateFusionSeAGradForward(
  ...
  FPTYPE* dy_dtwo = (dy_dtwo_tensor == nullptr) ? nullptr : dy_dtwo_tensor.view({-1}).data_ptr<FPTYPE>();
...
void TabulateFusionSeAGradGradForward(
  ...
  const FPTYPE* dz_dy_dtwo = (dz_dy_dtwo_tensor == nullptr) ? nullptr : dz_dy_dtwo_tensor.view({-1}).data_ptr<FPTYPE>();
  const int64_t nloc = em_tensor.size(0);
  const int64_t nnei = em_tensor.size(1);
  const int64_t last_layer_size = descriptor_tensor.size(2);
  // compute
...
template <typename FPTYPE>
void TabulateFusionSeTGradForward(
  const torch::Tensor& table_tensor,
  const torch::Tensor& table_info_tensor,
  const torch::Tensor& em_x_tensor,
  const torch::Tensor& em_tensor,
...
template <typename FPTYPE>
void TabulateFusionSeTGradGradForward(
  const torch::Tensor& table_tensor,
  const torch::Tensor& table_info_tensor,
  const torch::Tensor& em_x_tensor,
  const torch::Tensor& em_tensor,
...
template <typename FPTYPE>
void TabulateFusionSeRGradForward(
  const torch::Tensor& table_tensor,
  const torch::Tensor& table_info_tensor,
  const torch::Tensor& em_tensor,
  const torch::Tensor& dy_tensor,
...
template <typename FPTYPE>
void TabulateFusionSeRGradGradForward(
  const torch::Tensor& table_tensor,
  const torch::Tensor& table_info_tensor,
  const torch::Tensor& em_tensor,
  const torch::Tensor& dz_dy_dem_tensor,
  ...
  public:
    ...
    static torch::autograd::variable_list backward_t(
      ...
      TabulateFusionSeAGradGradForward<FPTYPE>(table_tensor, table_info_tensor, em_x_tensor,
                                              em_tensor, nullptr, dz_dy_dem_x_tensor,
                                              dz_dy_dem_tensor, nullptr, descriptor_tensor,
                                              is_sorted, dz_dy_tensor);

      return torch::autograd::variable_list{at::Tensor(), at::Tensor(), at::Tensor(),
                                            at::Tensor(), dz_dy_tensor, at::Tensor()};
    }
...
class TabulateFusionSeTOp : public torch::autograd::Function<TabulateFusionSeTOp>{
  public:
    static std::vector<torch::Tensor> forward(
      torch::autograd::AutogradContext* ctx,
      const torch::Tensor& table_tensor,
    ...
    static torch::autograd::variable_list forward_t(
      ...
      auto options = 
        torch::TensorOptions()
          ...
          .device(table_tensor.device());
      torch::Tensor descriptor_tensor = torch::empty(
        {em_tensor.size(0), last_layer_size}, options);
      // compute
      TabulateFusionSeTForward<FPTYPE>(table_tensor, table_info_tensor, em_x_tensor,
    ...
    static torch::autograd::variable_list backward_t(
      ...
      torch::Tensor dy_dem_tensor = torch::zeros_like(em_tensor);
      // compute
      TabulateFusionSeTGradForward<FPTYPE>(table_tensor, table_info_tensor, em_x_tensor,
...
class TabulateFusionSeROp : public torch::autograd::Function<TabulateFusionSeROp>{
  public:
    static std::vector<torch::Tensor> forward(
      torch::autograd::AutogradContext* ctx,
      const torch::Tensor& table_tensor,
    ...
    static torch::autograd::variable_list backward_t(
      ...
      torch::Tensor dy_dem_tensor = torch::zeros_like(em_tensor);
      // compute
      TabulateFusionSeRGradForward<FPTYPE>(table_tensor, table_info_tensor, em_tensor,
                                          dy_tensor, descriptor_tensor, dy_dem_tensor);
...
std::vector<torch::Tensor> tabluate_fusion_se_atten(
  ...
  return TabulateFusionSeAttenOp::apply(table_tensor, table_info_tensor, em_x_tensor,
                                        em_tensor, two_embed_tensor, last_layer_size,
                                        is_sorted);
}
...
std::vector<torch::Tensor> tabluate_fusion_se_t(
  ...
  return TabulateFusionSeTOp::apply(table_tensor, table_info_tensor, em_x_tensor,
                                    em_tensor, last_layer_size);
}
...
TORCH_LIBRARY_FRAGMENT(deepmd, m) { m.def("tabulate_fusion_se_atten", tabluate_fusion_se_atten); }
TORCH_LIBRARY_FRAGMENT(deepmd, m) { m.def("tabulate_fusion_se_t", tabluate_fusion_se_t); }
TORCH_LIBRARY_FRAGMENT(deepmd, m) { m.def("tabulate_fusion_se_r", tabluate_fusion_se_r); }