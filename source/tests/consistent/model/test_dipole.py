...
if INSTALLED_PT_EXPT:
    ...
    from deepmd.pt_expt.model import DipoleModel as DipoleModelPTExpt
else:
    DipoleModelPTExpt = None
...
class TestDipoleModelAPIs(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.pt_model = DipoleModelPT.deserialize(serialized)
        self.pt_expt_model = DipoleModelPTExpt.deserialize(serialized)
    ...
    def test_get_out_bias(self) -> None:
        ...
        DipoleModel's apply_out_stat is a no-op, so bias won't affect output,
        but the bias storage should still be consistent across backends.
        """
        ...
        # Verify shape: (n_output_keys x ntypes x 3) for dipole
    ...
    def test_do_grad_r(self) -> None:
        ...
        self.assertEqual(
            self.dp_model.do_grad_r("dipole"),
            self.pt_model.do_grad_r("dipole"),
        )
        self.assertTrue(self.dp_model.do_grad_r("dipole"))

    def test_do_grad_c(self) -> None:
        """do_grad_c should return the same value on dp and pt."""
        self.assertEqual(
            self.dp_model.do_grad_c("dipole"),
            self.pt_model.do_grad_c("dipole"),
        )
        self.assertTrue(self.dp_model.do_grad_c("dipole"))
    ...
    def test_change_out_bias(self) -> None:
        ...
        Note: DipoleModel's apply_out_stat is a no-op, so bias doesn't affect output,
        but the bias storage and fitting input stats should still be consistent.
        """
        nframes = 2
        nloc = 6
        numb_fparam = 2
        ...
        dipole_data = rng.normal(size=(nframes, 3)).astype(GLOBAL_NP_FLOAT_PRECISION)
        ...
        pt_merged = [
            {
                ...
                "dipole": numpy_to_torch(dipole_data),
                "find_dipole": np.float32(1.0),
    ...
    def test_change_type_map(self) -> None:
        ...
        dp_model = get_model_dp(data)
        pt_model = DipoleModelPT.deserialize(dp_model.serialize())
    ...
    def test_change_type_map_extend_stat(self) -> None:
        ...
        pt_small = DipoleModelPT.deserialize(dp_small.serialize())
        pt_large = DipoleModelPT.deserialize(dp_large.serialize())
        pt_expt_small = DipoleModelPTExpt.deserialize(dp_small.serialize())
        pt_expt_large = DipoleModelPTExpt.deserialize(dp_large.serialize())
    ...
    def test_compute_or_load_out_stat(self) -> None:
        ...
        Note: DipoleModel's apply_out_stat is a no-op, so bias doesn't affect
        output, but the stored bias should still be consistent.
        """
        import tempfile
        from pathlib import (
            Path,
...
class TestDipoleComputeOrLoadStat(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        dipole_stat = rng.normal(size=(nframes, 3)).astype(GLOBAL_NP_FLOAT_PRECISION)
        ...
        pt_sample = {
            ...
            "dipole": numpy_to_torch(dipole_stat),
            "find_dipole": np.float32(1.0),
    ...
    def test_compute_stat(self) -> None:
        ...
        for key in ("global_dipole", "dipole"):
            np.testing.assert_allclose(
                dp_ret0[key],
                pt_ret0[key],
                rtol=1e-10,
        ...
        # Note: DipoleModel's apply_out_stat is a no-op, so output won't change
        # after stat computation, but we still verify cross-backend consistency.
    ...
    def test_load_stat_from_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pt_model2 = DipoleModelPT.deserialize(dp_model2.serialize())
            pe_model2 = DipoleModelPTExpt.deserialize(dp_model2.serialize())
