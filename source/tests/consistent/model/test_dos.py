...
class TestDOSModelAPIs(unittest.TestCase):
    ...
    def test_get_out_bias(self) -> None:
        ...
        DOSModel uses default apply_out_stat (per-atom type bias),
        ...
        # Verify shape: (n_output_keys x ntypes x numb_dos) for dos
    ...
    def test_do_grad_r(self) -> None:
        ...
        self.assertEqual(
            self.dp_model.do_grad_r("dos"),
            self.pt_model.do_grad_r("dos"),
        )
        self.assertFalse(self.dp_model.do_grad_r("dos"))

    def test_do_grad_c(self) -> None:
        """do_grad_c should return the same value on dp and pt."""
        self.assertEqual(
            self.dp_model.do_grad_c("dos"),
            self.pt_model.do_grad_c("dos"),
        )
        self.assertFalse(self.dp_model.do_grad_c("dos"))
    ...
    def test_set_case_embd(self) -> None:
        ...
        pt_model = DOSModelPT.deserialize(serialized)
        pe_model = DOSModelPTExpt.deserialize(serialized)
        ...
        self.assertFalse(
            np.allclose(dp0["dos"], dp1["dos"]),
            "set_case_embd(0) and set_case_embd(1) produced the same dos",
        )
    ...
    def test_change_out_bias(self) -> None:
        ...
        dos_data = rng.normal(size=(nframes, 2)).astype(GLOBAL_NP_FLOAT_PRECISION)
    ...
    def test_change_type_map(self) -> None:
        ...
        dp_model = get_model_dp(data)
        pt_model = DOSModelPT.deserialize(dp_model.serialize())
    ...
    def test_change_type_map_extend_stat(self) -> None:
        ...
        pt_small = DOSModelPT.deserialize(dp_small.serialize())
        pt_large = DOSModelPT.deserialize(dp_large.serialize())
        pt_expt_small = DOSModelPTExpt.deserialize(dp_small.serialize())
        pt_expt_large = DOSModelPTExpt.deserialize(dp_large.serialize())
    ...
    def test_get_observed_type_list(self) -> None:
        ...
        # DOS uses get_compute_stats_distinguish_types()=True (default),
        # so only observed types get non-zero bias — only type 0 ("O").
...
class TestDOSComputeOrLoadStat(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        dos_stat = rng.normal(size=(nframes, 2)).astype(GLOBAL_NP_FLOAT_PRECISION)
    ...
    def test_compute_stat(self) -> None:
        ...
        # DOSModel uses default apply_out_stat (per-atom type bias), so
    ...
    def test_load_stat_from_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pt_model2 = DOSModelPT.deserialize(dp_model2.serialize())
            pe_model2 = DOSModelPTExpt.deserialize(dp_model2.serialize())
