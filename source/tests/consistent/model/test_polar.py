...
class TestPolarModelAPIs(unittest.TestCase):
    ...
    def test_get_out_bias(self) -> None:
        ...
        PolarModel's apply_out_stat applies diagonal bias with scale,
        so the bias storage should be consistent across backends.
        """
        ...
        # Verify shape: (n_output_keys x ntypes x 9) for polar
    ...
    def test_do_grad_r(self) -> None:
        ...
        self.assertEqual(
            self.dp_model.do_grad_r("polarizability"),
            self.pt_model.do_grad_r("polarizability"),
        )
        self.assertFalse(self.dp_model.do_grad_r("polarizability"))

    def test_do_grad_c(self) -> None:
        """do_grad_c should return the same value on dp and pt."""
        self.assertEqual(
            self.dp_model.do_grad_c("polarizability"),
            self.pt_model.do_grad_c("polarizability"),
        )
        self.assertFalse(self.dp_model.do_grad_c("polarizability"))
    ...
    def test_set_case_embd(self) -> None:
        ...
        pt_model = PolarModelPT.deserialize(serialized)
        pe_model = PolarModelPTExpt.deserialize(serialized)
        ...
        self.assertFalse(
            np.allclose(dp0["global_polar"], dp1["global_polar"]),
            "set_case_embd(0) and set_case_embd(1) produced the same global_polar",
        )
    ...
    def test_change_out_bias(self) -> None:
        ...
        polar_data = rng.normal(size=(nframes, 9)).astype(GLOBAL_NP_FLOAT_PRECISION)
        ...
        pt_merged = [
            {
                ...
                "polarizability": numpy_to_torch(polar_data),
                "find_polarizability": np.float32(1.0),
    ...
    def test_change_type_map(self) -> None:
        ...
        dp_model = get_model_dp(data)
        pt_model = PolarModelPT.deserialize(dp_model.serialize())
    ...
    def test_change_type_map_extend_stat(self) -> None:
        ...
        pt_small = PolarModelPT.deserialize(dp_small.serialize())
        pt_large = PolarModelPT.deserialize(dp_large.serialize())
        pt_expt_small = PolarModelPTExpt.deserialize(dp_small.serialize())
        pt_expt_large = PolarModelPTExpt.deserialize(dp_large.serialize())
    ...
    def test_compute_or_load_out_stat(self) -> None:
        ...
        PolarModel's apply_out_stat applies diagonal bias with scale,
        so the stored bias should be consistent across backends.
        """
        import tempfile
        from pathlib import (
            Path,
...
class TestPolarComputeOrLoadStat(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        polar_stat = rng.normal(size=(nframes, 9)).astype(GLOBAL_NP_FLOAT_PRECISION)
        ...
        pt_sample = {
            ...
            "polarizability": numpy_to_torch(polar_stat),
            "find_polarizability": np.float32(1.0),
    ...
    def test_compute_stat(self) -> None:
        ...
        # PolarModel's apply_out_stat applies diagonal bias with scale, so
        # output values WILL differ from pre-stat after stat computation.
    ...
    def test_load_stat_from_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pt_model2 = PolarModelPT.deserialize(dp_model2.serialize())
            pe_model2 = PolarModelPTExpt.deserialize(dp_model2.serialize())
