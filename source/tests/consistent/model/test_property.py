...
class TestPropertyModelAPIs(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.pt_model = PropertyModelPT.deserialize(serialized)
        self.pt_expt_model = PropertyModelPTExpt.deserialize(serialized)
    ...
    def test_get_out_bias(self) -> None:
        ...
        # Verify shape: (n_output_keys x ntypes x task_dim) for property
    ...
    def test_do_grad_r(self) -> None:
        ...
        self.assertEqual(
            self.dp_model.do_grad_r("foo"),
            self.pt_model.do_grad_r("foo"),
        )
        self.assertFalse(self.dp_model.do_grad_r("foo"))

    def test_do_grad_c(self) -> None:
        """do_grad_c should return the same value on dp and pt."""
        self.assertEqual(
            self.dp_model.do_grad_c("foo"),
            self.pt_model.do_grad_c("foo"),
        )
        self.assertFalse(self.dp_model.do_grad_c("foo"))
    ...
    def test_set_case_embd(self) -> None:
        ...
        pt_model = PropertyModelPT.deserialize(serialized)
        pe_model = PropertyModelPTExpt.deserialize(serialized)
        ...
        self.assertFalse(
            np.allclose(dp0["foo"], dp1["foo"]),
            "set_case_embd(0) and set_case_embd(1) produced the same foo",
        )
    ...
    def test_change_out_bias(self) -> None:
        ...
        foo_data = rng.normal(size=(nframes, 1)).astype(GLOBAL_NP_FLOAT_PRECISION)
    ...
    def test_change_type_map(self) -> None:
        ...
        dp_model = get_model_dp(data)
        pt_model = PropertyModelPT.deserialize(dp_model.serialize())
    ...
    def test_change_type_map_extend_stat(self) -> None:
        ...
        pt_small = PropertyModelPT.deserialize(dp_small.serialize())
        pt_large = PropertyModelPT.deserialize(dp_large.serialize())
        pt_expt_small = PropertyModelPTExpt.deserialize(dp_small.serialize())
        pt_expt_large = PropertyModelPTExpt.deserialize(dp_large.serialize())
...
class TestPropertyComputeOrLoadStat(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        foo_stat = rng.normal(size=(nframes, 1)).astype(GLOBAL_NP_FLOAT_PRECISION)
    ...
    def test_compute_stat(self) -> None:
        ...
        # PropertyModel's apply_out_stat applies output * std + bias, so
    ...
    def test_load_stat_from_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pt_model2 = PropertyModelPT.deserialize(dp_model2.serialize())
            pe_model2 = PropertyModelPTExpt.deserialize(dp_model2.serialize())
