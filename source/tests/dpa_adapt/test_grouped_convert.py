...
from dpa_adapt.data.grouped_convert import add_group_markers
...
def test_group_by_system_writes_group_id_and_pool_mask(tmp_path):
    ...
    results = add_group_markers(tmp_path, property_name="overpotential")
...
def test_group_by_label_splits_distinct_labels(tmp_path):
    ...
    add_group_markers(tmp_path, group_by="label", property_name="overpotential")
    gid = np.load(set_dir / "group_id.npy")
...
def test_group_by_int_chunks(tmp_path):
    ...
    add_group_markers(tmp_path, group_by=3)
    gid = np.load(set_dir / "group_id.npy")
...
def test_no_masked_atoms_skips_pool_mask(tmp_path):
    ...
    r = add_group_markers(tmp_path)[0]
...
def test_overwrite_false_preserves_existing(tmp_path):
    ...
    add_group_markers(tmp_path, property_name="overpotential", overwrite=False)
    assert np.load(set_dir / "group_id.npy").tolist() == [7, 7, 7]  # untouched

    add_group_markers(tmp_path, property_name="overpotential", overwrite=True)
...
def test_dry_run_writes_nothing(tmp_path):
    ...
    results = add_group_markers(tmp_path, property_name="overpotential", dry_run=True)
...
def test_no_systems_found_raises(tmp_path):
    ...
    with pytest.raises(DPADataError):
        add_group_markers(tmp_path / "empty")
