...
def _make_set_dir(
    set_dir, *, coord=None, box=None, energy=None, force=None, n_frames=3, n_atoms=2
):
...
def test_energy_nan_is_error(tmp_path):
    ...
    issues = check_data(system)
    assert any("energies" in i.file and "non-finite" in i.description for i in issues)
...
def test_force_inf_is_error(tmp_path):
    ...
    issues = check_data(system)
    assert any("forces" in i.file and "non-finite" in i.description for i in issues)


def test_box_nan_is_error(tmp_path):
    system = _system(tmp_path, box=np.full((3, 9), np.nan))
    issues = check_data(system)
    assert any("cells" in i.file and "non-finite" in i.description for i in issues)
...
def test_degenerate_box_is_error_with_det_in_description(tmp_path):
    ...
    issues = check_data(system)
    assert any("cells" in i.file and "degenerate" in i.description for i in issues)
...
def test_energy_magnitude_warning(tmp_path):
    ...
    issues = check_data(system)
    assert any(
        "energies" in i.file and "suspicious magnitude" in i.description for i in issues
    )
...
def test_force_magnitude_warning(tmp_path):
    ...
    issues = check_data(system)
    assert any(
        "forces" in i.file and "suspicious magnitude" in i.description for i in issues
    )
...
def test_frame_count_mismatch_is_error(tmp_path):
    ...
    issues = check_data(system)
    assert any(
        "energies" in i.file and "frame counts must align" in i.description
        for i in issues
    )
