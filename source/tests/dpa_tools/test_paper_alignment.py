...
from deepmd.dpa_tools.config.manager import MFTConfigManager
...
def _lp_config(tmp_path):
    ...
    ckpt.write_bytes(b"")
    t = _trainer(str(ckpt), tmp_path, freeze_backbone=True, output_dir=str(tmp_path / "o"))
...
def _ft_config(tmp_path):
    ckpt = tmp_path / "ckpt.pt"
    ckpt.write_bytes(b"")
    t = _trainer(str(ckpt), tmp_path, freeze_backbone=False, output_dir=str(tmp_path / "o"))
    with _patch_torch_load():
        config = t._build_config()
    return json.loads(json.dumps(config)), t
...
class _PropertyTuner:
    ...
    fitting_net_params = {
        "type": "ener", "neuron": [240, 240, 240], "dim_case_embd": 31, "seed": 1,
    }
...
_LEGACY_ENER_EXPECTED = {
    ...
    "learning_rate": {
        "type": "exp", "start_lr": 1e-3, "stop_lr": 1e-5, "decay_steps": 5000
    },
    "loss_dict": {
        "MP_traj_v024_alldata_mixu": {
            "type": "ener",
            ...
            "start_pref_v": 0.02, "limit_pref_v": 1
        },
        "DOWNSTREAM": {
            "type": "ener",
            "start_pref_e": 0.2, "limit_pref_e": 20,
    ...
    "training": {
        ...
        "data_dict": {
            ...
            "DOWNSTREAM": {
                "training_data": {"systems": ["/data/downstream"], "batch_size": "auto:32"}
            }
        },
        "numb_steps": 1000, "save_freq": 500, "disp_freq": 100, "seed": 42
    }
}
