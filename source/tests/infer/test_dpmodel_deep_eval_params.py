...
NFRAMES = 2
COORDS = np.stack((COORD, COORD + 0.125))
...
def test_shared_fparam_is_tiled_before_batching(
    ...
    # Evaluate shorthand first on a fresh batcher. On GPU, a successful
    # reference call would otherwise grow the batch size and remove the split
    # this regression is intended to exercise.
    actual = _backend(model_file, auto_batch_size).eval(
        COORDS,
        BOXES,
        ATOM_TYPES,
    ...
    expected = _backend(model_file, auto_batch_size=False).eval(
        COORDS,
        BOXES,
        ATOM_TYPES,
...
def test_distinct_full_parameters_preserve_frame_order_when_split(
    model_file: str,
) -> None:
    """Split evaluation preserves distinct frame-major parameter rows."""
    fparam = np.stack((FPARAM, FPARAM + np.array([0.4, -0.2])))
    aparam = np.stack((APARAM_PER_ATOM, APARAM_PER_ATOM + 0.35))

    split = _backend(model_file, auto_batch_size=len(ATOM_TYPES)).eval(
        COORDS,
        BOXES,
        ATOM_TYPES,
        fparam=fparam,
    ...
    unsplit = _backend(model_file, auto_batch_size=False).eval(
        COORDS,
        BOXES,
        ATOM_TYPES,
        fparam=fparam,
