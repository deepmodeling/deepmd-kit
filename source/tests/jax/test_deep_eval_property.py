...
"""Property-model inference on the JAX backend (StableHLO export).

Mirrors the dpmodel property test.  The JAX evaluator wraps an ``HLO`` object
with no live model, so the property variable name, output dimension and
intensiveness must be carried through the StableHLO export for the generic
``DeepEval`` factory to dispatch to ``DeepProperty`` and reshape the output.
"""
...
class TestDeepPropertyJAX(unittest.TestCase):
    def setUp(self) -> None:
        ...
        model_file = str(Path(self.tmpdir.name) / "property.hlo")
        deserialize_to_file(
            model_file,
    ...
    def test_model_type_dispatch(self) -> None:
        # The generic factory must resolve a property .hlo to DeepProperty
