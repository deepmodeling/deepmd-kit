...
def eval_pd_descriptor(
    ...
    ext_coords, ext_atype, mapping = extend_coord_with_ghosts_pd(
        paddle.from_numpy(coords).to(PD_DEVICE).reshape(1, -1, 3),
        paddle.from_numpy(atype).to(PD_DEVICE).reshape(1, -1),
        paddle.from_numpy(box).to(PD_DEVICE).reshape(1, 3, 3),
        pd_obj.get_rcut(),
    )
    ...
    result, _, _, _, _ = pd_obj(ext_coords, ext_atype, nlist, mapping=mapping)
    return result
...
class TestDescriptorSeA(unittest.TestCase):
    ...
    def test_compressed_forward(self) -> None:
        ...
        result_pd_compressed = eval_pd_descriptor(
            self.se_a,
            self.natoms,
            self.coords,
            self.atype,
