...
def _assert_runtime_profile() -> dict[str, bool]:
    ...
    policy_checks = {
        ...
        "supported": runtime_policy.is_supported_so2_capability(capability),
        "packed_wigner": runtime_policy.is_packed_wigner_enabled(capability),
    }
    config = so2._architecture_default_config(capability)
    if capability in SM80_CAPABILITIES:
        architecture_checks = {
            ...
            "thin_wrapper": runtime_policy.is_so2_thin_wrapper_enabled(capability),
...
def _run_efs_child(
    *,
    ...
    try:
        ...
        if use_compile:
            so2._build_runner = original_build_runner
    ...
    if use_compile and runner_calls == 0:
        raise AssertionError("compiled CuTe run did not instantiate the SO2 runner")
    torch.save(
        {
            "outputs": runs,
            "profile": profile,
