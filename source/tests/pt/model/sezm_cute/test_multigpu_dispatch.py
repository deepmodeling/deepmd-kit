...
def test_so2_custom_ops_are_direct_and_thin_path_uses_operand_device() -> None:
    operand = _cuda_operand()
    sentinel = object()

    with mock.patch.object(so2, "_cute_so2_impl", return_value=sentinel) as direct:
        actual = so2.cute_so2(
            1,
            operand,
            object(),
    ...
    with mock.patch.object(
        so2,
        "_so2_packed_direct_registered_backward_impl",
        return_value=sentinel,
    ) as backward:
        actual = so2._so2_packed_direct_backward(context, operand, None)
    ...
    with (
        ...
        mock.patch.object(
            so2,
            "_maybe_run_cute_so2_fallback",
            return_value=sentinel,
        ) as fallback,
    ):
        actual = so2.maybe_run_cute_so2(object(), operand, object(), object())
...
def test_so2_prepare_uses_requested_device_instead_of_current_device() -> None:
    ...
    with (
        ...
        mock.patch.object(
            so2,
            "is_supported_so2_compute_capability",
            return_value=False,
        ) as supported,
        mock.patch.object(
            torch.cuda,
        ...
        assert not so2.prepare_cute_so2_blocks(
            [object()],
            training=False,
            device=device,
            dtype=torch.float32,
...
def test_wignerd_custom_ops_are_called_directly() -> None:
    pytest.importorskip("cutlass.cute")
    wignerd = importlib.import_module("deepmd.pt_expt.kernels.cute.sezm.wignerd")
    operand = _cuda_operand()
    sentinel = object()
    ...
    with mock.patch.object(
        ...
        actual = wignerd.run_cute_wignerd(operand, object())
    ...
    with mock.patch.object(
        ...
        actual = wignerd._wignerd_panel_backward(context, grad_panel)
...
def test_mlp_selector_and_forward_use_input_device() -> None:
    ...
    with (
        ...
        assert mlp._use_so2_compile_visible_linear(cuda1)
        assert not mlp._use_so2_compile_visible_linear(torch.device("cuda", 0))
        assert not mlp._use_so2_compile_visible_linear(torch.device("cpu"))
    ...
    with mock.patch.object(
        mlp,
        "_use_so2_compile_visible_linear",
        return_value=False,
    ) as selector:
        layer(value)
