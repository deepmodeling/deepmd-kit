...
def test_exact_structure_and_frozen_inference_guards():
    ...
    module = _output_ffn()
    assert readout_l0.has_neo_readout_contract(module)
    assert readout_l0._inference_mode_is_frozen(module)
    assert not readout_l0.has_neo_readout_contract(_output_ffn(hidden_channels=64))
    module.train()
...
def test_descriptor_readout_wiring(cpu_neo_descriptor, monkeypatch):
    from deepmd.pt_expt.kernels.cute.sezm.output_grid import (
        readout_l0,
    )
    ...
    def record_readout(output_ffn, ffn_in):
        calls.append((output_ffn, ffn_in))
        return expected

    monkeypatch.setenv("DP_CUTE_INFER", "1")
    monkeypatch.setattr(readout_l0, "maybe_run_neo_readout_l0", record_readout)
    assert descriptor.run_cute_infer_readout(value) is expected
    assert calls[-1] == (descriptor.output_ffn, value)
