...
"""Contracts for the opt-in CuTe Neo SO2 inference path."""
...
from deepmd.pt_expt.kernels.cute.sezm.so2 import operation as _SO2
from deepmd.pt_expt.kernels.cute.sezm.so2 import runner as _SO2_RUNNER

NeoSO2BackwardWorkspace = _SO2_RUNNER.NeoSO2BackwardWorkspace
NeoSO2RuntimeConfig = _SO2.NeoSO2RuntimeConfig
NeoFullCuteBackward = _SO2_RUNNER.NeoFullCuteBackward
...
def test_non_neo_contracts_fall_back(field: str, value: object) -> None:
    assert not _SO2.is_supported_neo_so2_block(_neo_like_block(**{field: value}))
...
def test_unsupported_message_grid_contract_falls_back() -> None:
    ...
    assert not _SO2.is_supported_neo_so2_block(block)
...
def test_gate_expand_contract_cache_tracks_buffer_versions() -> None:
    ...
    assert _SO2._gate_expand_index_structure_is_supported(block)
    assert _SO2._gate_expand_index_is_supported(block)
    expand_index[0] = 2
    assert _SO2._gate_expand_index_structure_is_supported(block)
    assert not _SO2._gate_expand_index_is_supported(block)


@pytest.mark.parametrize(
    "capability",
    sorted(runtime_policy.SUPPORTED_SO2_CAPABILITIES),
)
def test_every_supported_architecture_has_a_validated_config(
    capability: tuple[int, int],
) -> None:
    config = _SO2._architecture_default_config(capability)
...
def test_sm90_uses_native_split_complex_path() -> None:
    config = _SO2._architecture_default_config((9, 0))
...
def test_sm100_uses_shared_default_profile() -> None:
    config = _SO2._architecture_default_config((10, 0))
...
def test_combined_so2_gate_rejects_misaligned_contiguous_tensor() -> None:
    ...
    from deepmd.pt_expt.kernels.cute.sezm.so2.kernels.combined_gate_forward import (
...
def test_runtime_config_contains_only_reached_selectors() -> None:
    assert {field.name for field in fields(NeoSO2RuntimeConfig)} == {
        "native_sm90_path",
...
def test_runtime_config_rejects_incompatible_capability() -> None:
    config = _SO2._architecture_default_config((8, 0))
...
def test_packed_edge_eligibility_requires_sorted_strict_fp32() -> None:
    ...
    assert _SO2.packed_wigner_edges_eligible(**kwargs)
    assert not _SO2.packed_wigner_edges_eligible(
        **{**kwargs, "destinations_sorted": False}
    )
    assert not _SO2.packed_wigner_edges_eligible(
        **{**kwargs, "runtime_dtypes": (torch.float64,)}
    )
    assert _SO2.packed_wigner_edges_eligible(
...
def test_ineligible_runtime_contracts_fall_back(
    ...
    assert not _SO2.is_neo_so2_runtime_eligible(
        block,
        training=training,
...
def test_runtime_contract_allows_fewer_edges_than_nodes(
    ...
    assert _SO2.is_neo_so2_runtime_eligible(
        block,
        training=False,
        device=torch.device("cuda", 0),
        dtype=torch.float32,
...
def test_exact_runtime_contract_is_eligible_without_autocast(
    ...
    assert _SO2.is_neo_so2_runtime_eligible(block, **kwargs)
...
def test_fake_native_gradient_layout_matches_runtime_contract() -> None:
    ...
    grad = _SO2._fake_x_wide_grad_like(x, skip=True)
...
def test_custom_op_runtime_canonicalizes_misaligned_contiguous_view() -> None:
    ...
    actual = _SO2._aligned_contiguous(offset_view)
...
def test_custom_op_runtime_preserves_aligned_compact_tensor() -> None:
    ...
    assert _SO2._aligned_contiguous(tensor) is tensor
