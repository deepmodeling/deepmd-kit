...
"""Contracts for asynchronous SO2 destination-pointer construction."""
...
SO2_PATH = CUTE_ROOT / "so2" / "operation.py"
SO2_METADATA_PATH = CUTE_ROOT / "so2" / "metadata.py"
...
def _load_extracted_dst_ptr_function(*, strict: bool = False):
    source = SO2_PATH.read_text(encoding="utf-8")
    ...
    exec(compile(module, str(SO2_PATH), "exec"), namespace)
    return namespace["_dst_ptr_from_sorted"]


def _load_extracted_sorted_metadata_function():
    source = SO2_METADATA_PATH.read_text(encoding="utf-8")
    tree = ast.parse(source)
...
class TestSO2DstPtrExtracted(unittest.TestCase):
...
class TestSO2DstPtrTorch(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    @unittest.skipUnless(
        ...
        "SO2 custom-op registry lifetime regression requires CUDA",
    )
    def test_compile_cold_registration_survives_into_packed_custom_op_runtime(self):
        ...
        config = so2.NeoSO2RuntimeConfig()
        runtime_handles = []
        ...
        def fake_build(handle, x_arg, *args):
            del args
            entry = so2._REGISTRY[int(handle)]
        ...
        def invoke(
            ...
            if state is None:
                state = so2._register_cute_so2_state(
                    block,
                    x_arg.device.index,
                    config,
                )
            dst_ptr = so2._dst_ptr_from_sorted(
                torch,
                dst_arg,
                x_arg.shape[0],
                destinations_sorted=True,
        ...
        with mock.patch.object(so2, "_build_runner", new=fake_build):
        ...
        state = block._deepmd_cute_so2_state
        self.assertEqual(runtime_handles, [state.handle, state.handle])
        self.assertEqual(list(so2._REGISTRY), [state.handle])
        self.assertIs(so2._REGISTRY[state.handle].block, block)
