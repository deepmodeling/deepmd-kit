...
KERNEL_PATH = CUTE_ROOT / "so2" / "kernels" / "structural_gate_sm80.py"
...
def _load_wrapper_module():
    return importlib.import_module(
        "deepmd.pt_expt.kernels.cute.sezm.so2.structural_gate"
    )
...
class TestSM80StructuralGateVec4Contract(unittest.TestCase):
    ...
    def test_kernel_is_strict_fp32_edge_major_only(self):
        ...
        self.assertIn("runtime_policy.SUPPORTED_SO2_CAPABILITIES", self.kernel_source)
...
@unittest.skipUnless(
    ...
    "A supported Neo SO2 CUDA runtime is required",
)
class TestSM80StructuralGateVec4CudaDifferential(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        from deepmd.pt_expt.kernels.cute.sezm.so2.kernels.structural_gate_sm80 import (
