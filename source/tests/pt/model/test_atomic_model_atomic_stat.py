...
class TestAtomicModelStat(unittest.TestCase, TestCaseSingleFrameWithNlist):
    ...
    def test_output_stat(self):
        ...
        expected_std = np.ones((2, 2, 2), dtype=np.float64)  # 2 keys, 2 atypes, 2 max dims.
        expected_std[0,:,:1] = np.array([0.0, 0.816496]).reshape(2,1) # updating std for foo based on [5.0, 5.0, 5.0], [5.0, 6.0, 7.0]]
        ...
        ret3 = cvt_ret(ret3)
        expected_std[0,:,:1] = np.array([1.24722, 0.47140]).reshape(2,1) # updating std for foo based on [4.0, 3.0, 2.0], [1.0, 1.0, 1.0]]
        expected_ret3 = {}
