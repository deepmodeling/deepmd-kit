...
"""Parity tests for the real-module adjoints used by Neo K1 backward."""
...
def test_equivariant_rmsnorm_manual_adjoint_matches_real_module() -> None:
    ...
    actual_grad = k1._equivariant_rmsnorm_backward(
        norm,
        x.detach(),
        grad_out,
    )
...
def test_focus_linear_manual_adjoint_matches_real_module() -> None:
    ...
    torch.testing.assert_close(k1._focus_linear_forward(linear, x), reference)
    torch.testing.assert_close(
        k1._focus_linear_backward_input(linear, grad_out),
        reference_grad,
        atol=1.0e-12,
        rtol=1.0e-12,
    )
...
def test_swiglu_manual_adjoint_matches_real_module() -> None:
    ...
    torch.testing.assert_close(k1._swiglu_forward(x), reference)
    torch.testing.assert_close(
        k1._swiglu_backward_input(x.detach(), grad_out),
        reference_grad,
        atol=1.0e-12,
        rtol=1.0e-12,
    )
...
def test_grid_cross_glu_flat_manual_adjoint_matches_real_module() -> None:
    ...
    actual_query, actual_context = k1._so3_grid_cross_glu_flat_backward(
        net,
        query.detach(),
        context.detach(),
        grad_out,
