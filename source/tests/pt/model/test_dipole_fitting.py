...
class TestEquivalence(unittest.TestCase):
    ...
    def test_rot(self):
        ...
        for distinguish_types, nfp, nap in itertools.product(
                ...
                for xyz in [self.coord, coord_rot]:
                    extended_coord, extended_atype, mapping, nlist = extend_input_and_build_neighbor_list(xyz + self.shift, atype, self.rcut, self.sel, distinguish_types)
                ...
                np.testing.assert_allclose(
                        ...
                        to_numpy_array(torch.matmul(res[0], rmat))
                    )
    ...
    def test_permu(self):
        ...
        for idx_perm in [[0, 1, 2, 3, 4], [1, 0, 4, 3, 2]]:
            ...
            extended_coord, extended_atype, mapping, nlist = extend_input_and_build_neighbor_list(coord[idx_perm], atype, self.rcut, self.sel, False)
    ...
    def test_trans(self):
        ...
        coord_s = torch.matmul(
            torch.remainder(torch.matmul(self.coord + self.shift, torch.linalg.inv(self.cell)), 1.0),
            self.cell,
        )
        ft0 = DipoleFittingNet(
                    "foo",
        ...
        for xyz in [self.coord, coord_s]:
            extended_coord, extended_atype, mapping, nlist = extend_input_and_build_neighbor_list(xyz + self.shift, atype, self.rcut, self.sel, False)
