...
@unittest.skipIf(not HAS_FINUFFT, "pytorch_finufft is required for LES tests")
class TestLESWorkingLayer(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.fitting = LESEnergyFittingNet(
            var_name="energy",
            ntypes=self.nt,
    ...
    def test_frame_correction_applies_once_per_frame(self) -> None:
        ...
        frame_corr = self.model._compute_les_frame_correction(
    ...
    def test_long_range_params_have_gradient_path(self) -> None:
        ...
        grads = torch.autograd.grad(
            out["energy"].sum(),
            [self.fitting.wl, self.fitting.sl],
            retain_graph=False,
            create_graph=False,
            allow_unused=False,
        )
        for gg in grads:
            ...
            self.assertTrue(torch.isfinite(gg).all().item())
