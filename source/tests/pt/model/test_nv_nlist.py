...
@unittest.skipUnless(
    torch.cuda.is_available() and nv_nlist.is_nv_available(),
    "NVIDIA Toolkit-Ops CUDA path is unavailable",
)
class TestNVNList(unittest.TestCase):
    ...
    def _build_case(
        ...
        atype = torch.tensor([[0, 1, 0, 1]], dtype=torch.int32, device=self.device)
    ...
    def _build_overfull_case(self) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        coord = torch.tensor(
            [
                [0.0, 0.0, 0.0],
                [0.9, 0.0, 0.0],
        ...
        box = torch.eye(3, dtype=torch.float64, device=self.device).reshape(1, 9) * 20.0
        return coord, atype, box
    ...
    def _assert_nv_matches_native(
        ...
        if force_cell_list:
            with mock.patch.object(nv_nlist, "NV_CELL_LIST_THRESHOLD", 1):
    ...
    def test_cell_list_matches_native(self) -> None:
        ...
        """
        coord, atype, box = self._build_case(2)
        self._assert_nv_matches_native(
            coord=coord, atype=atype, box=box, rcut=3.0, sel=[8], force_cell_list=True
        )
    ...
    def test_overfull_truncates_to_sel(self) -> None:
        ...
        """
        coord, atype, box = self._build_overfull_case()
        self._assert_nv_matches_native(
            coord=coord, atype=atype, box=box, rcut=4.0, sel=[2], force_cell_list=False
        )

    def test_requires_periodic_box(self) -> None:
        """The cell list needs a periodic box; ``box=None`` is rejected."""
        coord, atype, _ = self._build_case(1)
