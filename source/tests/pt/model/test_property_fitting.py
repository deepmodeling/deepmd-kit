...
class TestInvarFitting(unittest.TestCase, TestCaseSingleFrameWithNlist):
    ...
    @unittest.skip("Don't need to test new&old in property fitting")
    def test_new_old(
        self,
    ):
        rng = np.random.default_rng()
        ...
        for foo, mixed_types in itertools.product(
            ...
            np.testing.assert_allclose(
                ...
                to_numpy_array(ret1["property"]),
            )

    @unittest.skip("Don't need to test jit at this time")
    def test_jit(
        self,
    ):
