...
class TestDescrptSeAVg(unittest.TestCase, TestCaseSingleFrameWithNlist):
    ...
    def test_dp_atomic_model_accepts_aparam_for_freeze(self) -> None:
        type_map = ["A", "B"]
        dd = DescrptSeAVg(
            self.rcut,
            self.rcut_smth,
        ...
        model = DPAtomicModel(dd, fitting, type_map).to(env.DEVICE)
        self.assertTrue(model._descriptor_accepts_aparam)
        scripted = torch.jit.script(model)
        self.assertTrue(scripted._descriptor_accepts_aparam)

    def test_scripted_descriptor_uses_aparam(self) -> None:
        prec = "float64"
        pt_dtype = PRECISION_DICT[prec]
        nf, nloc, _ = self.nlist.shape
        dd = DescrptSeAVg(
        ...
        scripted = torch.jit.script(dd)
        out0_jit, _, _, _, _ = scripted(coord, atype_ext, nlist, aparam=aparam_zero)
        out1_jit, _, _, _, _ = scripted(coord, atype_ext, nlist, aparam=aparam_one)
        self.assertGreater((out0_jit - out1_jit).abs().max().item(), 0.0)
        torch.testing.assert_close(out0, out0_jit)
        torch.testing.assert_close(out1, out1_jit)
