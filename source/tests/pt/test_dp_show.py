...
class TestSingleTaskModel(unittest.TestCase):
    ...
    def test_checkpoint(self):
        ...
        assert ("{'type': 'se_e2_a'" and "'sel': [46, 92, 4]" and "'rcut': 4.0") in log_messages[2] 
        assert log_messages[3] == "The fitting_net parameter is {'neuron': [24, 24, 24], 'resnet_dt': True, 'seed': 1}"
...
class TestMultiTaskModel(unittest.TestCase):
    def setUp(self):
        ...
        with open(input_json) as f:
            self.config = json.load(f)
        self.config["model"]["shared_dict"]["my_descriptor"] = model_se_e2_a["descriptor"]
    ...
    def test_checkpoint(self):
        ...
        assert ("model_1" and "'type': 'se_e2_a'" and "'sel': [46, 92, 4]" and "'rcut_smth': 0.5") in log_messages[4]
        assert ("model_2" and "'type': 'se_e2_a'" and "'sel': [46, 92, 4]" and "'rcut_smth': 0.5") in log_messages[5]
        assert log_messages[6] == "The fitting_net parameter of branch model_1 is {'neuron': [1, 2, 3], 'seed': 678}"
        assert log_messages[7] == "The fitting_net parameter of branch model_2 is {'neuron': [9, 8, 7], 'seed': 1111}"
    ...
    def test_frozen_model(self):
        ...
        assert ("'type': 'se_e2_a'" and "'sel': [46, 92, 4]" and "'rcut_smth': 0.5") in log_messages[2]
        assert log_messages[3] == "The fitting_net parameter is {'neuron': [1, 2, 3], 'seed': 678}"
