...
class FinetuneTest:
    def test_finetune_change_out_bias(self):
        ...
        energy_small = dp.eval(
            ...
            to_numpy_array(
                sampled[1]["box"][:ntest] if sampled[1]["box"] is not None else None
            ),
        ...
        if self.testkey == "global_dipole":
            np.testing.assert_almost_equal(
                energy_bias_after[idx_type_map], energy_bias_before[idx_type_map]
            )  # diploe model not applying bias
        else:
            np.testing.assert_almost_equal(
                finetune_shift, ground_truth_shift, decimal=10
            )
        self.tearDown()
    ...
    def setUp(self):
        ...
        self.data_file = [
            str(
                ...
                / "examples/water_tensor/dipole/training_data_reformat/global_system"
            )
        ]
        self.config["training"]["training_data"]["systems"] = self.data_file
        self.config["training"]["validation_data"]["systems"] = self.data_file
