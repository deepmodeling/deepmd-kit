...
class TestDOSFittingNet(unittest.TestCase, TestCaseSingleFrameWithNlist):
    def setUp(self) -> None:
    ...
    def test_self_consistency(self) -> None:
        ...
        for nfp, nap in [(0, 0), (3, 0), (0, 4), (3, 4)]:
            ...
            fn1 = DOSFittingNet.deserialize(fn0.serialize()).to(self.device)
            if nfp > 0:
            ...
            np.testing.assert_allclose(
                ret0["dos"].detach().cpu().numpy(),
                ret1["dos"].detach().cpu().numpy(),
            )
    ...
    def test_serialize_has_correct_type(self) -> None:
        ...
        self.assertEqual(serialized["type"], "dos")
        fn2 = DOSFittingNet.deserialize(serialized).to(self.device)
        self.assertIsInstance(fn2, DOSFittingNet)
    ...
    def test_torch_export_simple(self) -> None:
        ...
        np.testing.assert_allclose(
            ret["dos"].detach().cpu().numpy(),
            ret_exported["dos"].detach().cpu().numpy(),
            rtol=1e-10,
            atol=1e-10,
        )
