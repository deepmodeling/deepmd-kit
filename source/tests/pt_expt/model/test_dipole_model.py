...
from deepmd.dpmodel.model.dipole_model import DipoleModel as DPDipoleModel
...
class TestDipoleModel(unittest.TestCase):
    ...
    def _make_dp_model(self):
        ...
        return DPDipoleModel(ds, ft, type_map=self.type_map)
    ...
    def test_dp_consistency(self) -> None:
        ...
        md_pt = DipoleModel.deserialize(md_dp.serialize()).to(self.device)
        md_pt.eval()
        ...
        np.testing.assert_allclose(
            ret_dp["global_dipole"],
            ret_pt["global_dipole"].detach().cpu().numpy(),
            rtol=1e-10,
            atol=1e-10,
        )
        np.testing.assert_allclose(
            ret_dp["dipole"],
            ret_pt["dipole"].detach().cpu().numpy(),
            rtol=1e-10,
            atol=1e-10,
        )
    ...
    def test_output_keys(self) -> None:
        ...
        self.assertIn("global_dipole", ret)
