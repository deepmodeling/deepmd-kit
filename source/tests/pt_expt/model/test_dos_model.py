...
from deepmd.dpmodel.fitting import DOSFittingNet as DPDOSFittingNet
from deepmd.dpmodel.model.dos_model import DOSModel as DPDOSModel
...
class TestDOSModel(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _make_dp_model(self):
        ...
        return DPDOSModel(ds, ft, type_map=self.type_map)
    ...
    def test_dp_consistency(self) -> None:
        ...
        md_pt = DOSModel.deserialize(md_dp.serialize()).to(self.device)
        md_pt.eval()
        ...
        np.testing.assert_allclose(
            ret_dp["dos"],
            ret_pt["dos"].detach().cpu().numpy(),
            rtol=1e-10,
            atol=1e-10,
        )
        np.testing.assert_allclose(
            ret_dp["atom_dos"],
            ret_pt["atom_dos"].detach().cpu().numpy(),
            rtol=1e-10,
            atol=1e-10,
        )
    ...
    def test_output_keys(self) -> None:
        ...
        self.assertIn("dos", ret)
        self.assertIn("atom_dos", ret)
    ...
    def test_forward_lower_exportable(self) -> None:
        ...
        # Compare loaded vs eager (nf=1 — different shapes)
