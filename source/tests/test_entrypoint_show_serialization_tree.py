...
class TestShowSerializationTree(unittest.TestCase):
    def test_serialization_tree_uses_backend_hook_model_payload(self) -> None:
        with (
            patch("deepmd.entrypoints.show.DeepEval") as mock_deep_eval,
            patch("deepmd.entrypoints.show.Backend.detect_backend_by_model") as mock_detect,
            ...
            backend = mock_detect.return_value.return_value
            backend.serialize_hook.return_value = {
                "backend": "PyTorch Exportable",
                "model": {"@class": "MockModel"},
                "model_def_script": {"type_map": ["H", "O"]},
                "@variables": {},
            }
            ...
            backend.serialize_hook.assert_called_once_with("mock.pte")
