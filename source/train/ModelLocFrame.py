...
class ModelLocFrame(Model) :
    ...
    def build_atom_net (self, 
        ...
        for type_i in range(self.ntypes):
            ...
            layer = self.one_layer(inputs_i, self.n_neuron[0], name='layer_0_type_'+str(type_i), reuse=reuse, seed = self.seed)
            for ii in range(1,len(self.n_neuron)) :
                layer = self.one_layer(layer, self.n_neuron[ii], name='layer_'+str(ii)+'_type_'+str(type_i), reuse=reuse, seed = self.seed)

            final_layer = self.one_layer(layer, 1, activation_fn = None, bavg = type_bias_ae, name='final_layer_type_'+str(type_i), reuse=reuse, seed = self.seed)
